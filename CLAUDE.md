...
1. **Coordination, not execution.** CommanderSCP does not deploy, provision, or run tests itself; executor integrations get only observe/trigger/status/abort verbs, and the platform does not hold credentials to the infrastructure that execution systems manage. One scoped exception (owner decision, 2026-07-08): the isolated `scp-managed-iac` executor performs trivial IaC releases for orgs without pipelines — behind the same executor interface, in ephemeral containers from a separate `scp-runner-iac` image, with vaulted scoped credentials (DESIGN.md §12; charter "Managed Execution Exception", approved 2026-07-08). Since 2026-09-25 (charter "Managed Standard Stack"), CommanderSCP also **installs, wires, upgrades and operates the Standard Stack it bundles** (Argo CD/Workflows/Rollouts/Events, Gitea), through a stack controller kept separate from the coordination server. This is software lifecycle of the executors, never credentials to the infrastructure they manage. **CommanderSCP is the surface, and automatic is the default**: users never operate a backend directly, and a step is manual only when a user configures it that way.
...
- **Census by property, not by symptom.** When you fix a bug, name the *property* that made it possible, then find every other place with that property before you ship. A bug found by a symptom is one instance; the property is the class. Two rules make this concrete: census with **no grep filters** (a filter is where the next instance hides), and treat a **well-written comment naming a hazard as a signal to sweep, not as evidence it was handled**. Worked example in BUILD_AND_TEST.md §4.4a — the same starvation property was found, correctly diagnosed, and fixed in *one* of four places; one of the three left behind stopped production coordination for 13 days behind green health checks.
- **Use `grep -rna`, never `grep -rn`, for a census.** Some tracked source files contain literal NUL bytes — NUL is a composite-key delimiter here, chosen because it cannot occur in the components being joined; it is correct and must not be "fixed". Every search tool classifies those files as binary and **drops them from a recursive search with no output and exit 1** — identical to "no such code exists". `grep -rna` and `rg --text` read them; `git grep` reads only files whose first NUL falls past its 8 KB sniff, so it is right by accident and changes its mind as files grow. `plan-diff.ts` is one of them and holds the sole label test that makes an object a *delete* candidate, so an IaC-deletion census run the documented way misses the only place that matters. `pnpm nul-census` prints the current set (details and measurements in BUILD_AND_TEST.md §4.4b).
- **A claim about a tool cannot be verified with that tool.** If the claim is "grep hides these files", grep is the one instrument that cannot settle it — its blind spot is the subject. This is not hypothetical: `grep -rlP '\x00'` returned a confident **zero** across 1107 files, which was reported as a disproof, used to dismiss the issue, and used to dispatch an agent to strip the true warning out of a PR. The bytes were always there; `grep -ralP '\x00'` lists all four, and a byte-level read finds them instantly. Step outside the tool — read bytes, check the exit code against a known-positive control, or use a second implementation — before you call a tool-behaviour claim false.
