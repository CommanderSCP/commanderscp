...
drizzle-orm's `sql` tag expands a JS array interpolation into a parenthesised parameter LIST, so `${roots}::uuid[]` receives a bare scalar or an anonymous record and fails to cast — measured and written down at `graph/sql-helpers.ts`. The in-tree idiom for a caller-sized set of typed parameters is `(VALUES (${id}::uuid), ...)` (`coordination-as-code/stack-ownership.ts`, `coordination/service-board.ts`), and it binds each id as its own parameter, so no id is ever concatenated into SQL text.
