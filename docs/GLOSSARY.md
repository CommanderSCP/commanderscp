...
**An org-tier freeze federates only if it says so; a platform-tier freeze never does.** *(Owner decision D6, 2026-08-23; [ADR-0043](adr/0043-federated-org-tier-freezes.md).)* A freeze at the **org tier or below** declared `federate: true` gains a `freeze` **graph object** and rides the **existing** `object_upsert` entry kind to this org's peers, where the importer rebuilds its projection row and the freeze **blocks there too**. It **defaults to `false`** — a new reach never defaults on — and the federating form is gated on **`federation:write` in addition to `freeze:write`**, because declaring a freeze that binds another security domain is not the same act as describing your own estate — and the same pair is required to **lift** it or move its `endsAt`, because both verbs re-publish the object. The replica is **read-only**: an outpost cannot lift or shorten a commander's freeze (**409**); its remedy is `freeze:override` at that freeze's own scope, per change, with a reason. An outpost declaring its own may mark it `domainLocal` ([ADR-0031](adr/0031-domain-local-objects-never-federate.md)), which withholds it in both directions. **It reaches peers paired at `full` sync scope only** — it rides an `object_upsert`, which `policies_only`, `status_only`, `changes_only` and a non-empty `custom` selector all exclude — and that drop is **silent at both ends**, so a freeze declared against a narrowly-scoped outpost blocks nothing there and neither instance reports it (ADR-0043 §5a).

*Until 2026-08-24 **no** freeze could cross a boundary at all, and that was deliberate rather than unbuilt: a freeze was a projection row with no graph object, `JournalEntryKindSchema` carries nothing freeze-shaped, the absence was pinned by a test, and the outpost UI explained it to operators verbatim. D6 retracted it for the org tier only.*

**In the code.** `freezes` and `instance_freezes` in `apps/server/src/db/schema.ts`; resolved by `governance/freeze-scope.ts`'s `freezesByTarget`; enforced at `governance/gate-orchestrator.ts`'s `checkFreeze` and, per target, at `coordination/freeze-hold.ts` + the reconciler's trigger loop. The federating half is `governance/freeze-object.ts` (the wire form and the import-side projection rebuild) and `freezes.object_id` (drizzle/0089). DESIGN.md §10.3.
...
**In the code — proposed, not yet built.** No dial-URL list, no `XO` label, and no `scp doctor` readiness check exist in the schema or CLI at time of writing; [ADR-0044](adr/0044-multi-region-instance-resilience.md) records the design, targeted at M26.3.
...
**In the code.** Not a stored value anywhere — it is documentation/runbook vocabulary for wherever an operator chooses to place a member cluster. See `docs/runbooks/resilience.md` (once built, [ADR-0044](adr/0044-multi-region-instance-resilience.md) D7) for per-role placement guidance.
