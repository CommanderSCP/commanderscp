...
`stack_backend_registrations.object_id` (M29.2, ADR-0061) is `reader-fails-closed` with no DELETE grant and no foreign key, because the id is allocated before the object exists. A Standard Stack registration cannot be tombstoned through any door: `assertStackRegistrationWrite` refuses every delete of it, the stack's own included. Unwiring or no longer serving the org makes it refuse to resolve instead. If one were tombstoned anyway, bindings resolve their target live before the stack routing runs, and the registration reconcile tries to re-create the same id, which the tombstoned row still holds, so it fails loudly rather than routing anything.

`execution_system_source_allowlists.execution_system_object_id` (M28.3, ADR-0056 §7a) is `reader-fails-closed` with no DELETE grant: the allowlist records which repos a system's credentials were allowed to run and outlives the system, and every reader resolves the LIVE system (`getSourceAllowlist` joins it; a tombstoned system allows nothing, and so does a live one whose routing fingerprint — its whole canonical `properties`, or #415's four fields for a row still at `routing_fingerprint_version` 1 — no longer matches the one the row was set for, ADR-0056 addendum 3).
...
`ops_run_redemptions.change_object_id` (M28.2, ADR-0054, migration 0121) is **historical**, with no DELETE grant: the row attributes an issued certificate serial to the Argo host-ops run that redeemed it, so it must outlive the change as the issuance row and the audit chain do. It cannot be acted on after the change is tombstoned — the redeem door locks the row by `(org, id)` and refuses once its window (the certificate's own TTL) has closed — so the dangle is inert by construction rather than by being unreached.
