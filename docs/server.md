...
### §106. Every `:idOrUrn`/`:urn` route param can carry a percent-encoded object URN

Every `:idOrUrn`/`:urn` route param can carry a percent-encoded object URN. Fastify/find-my-way's `maxParamLength` is a ROUTER-level ceiling on one path-parameter segment's raw length, checked BEFORE route matching — an over-long segment never reaches its Zod `params` schema at all, so no amount of widening that schema helps. Measured live 2026-09-19: `DELETE /api/v1/executors/{urn}/binding` 414d for a real placement URN (org uuid + component name + `/` + stage name, percent-encoded) under the find-my-way default of 100 chars; the identical call with the owner object's UUID returned 200. Census (grep `params: <SchemaName>` with no filter across `apps/server/src/routes`, for every URN-capable param schema — `RegistryIdOrUrnParamSchema`, `ObjectIdOrUrnParamSchema`, `RegistryUrnParamSchema`, `ObjectUrnParamSchema`, `RegistryOwnerParamSchema`, `RegistryTargetParamSchema`, `RungParamSchema`, `ObjectEnforcementParamSchema`, federation's inline overlay `idOrUrn`): 42 route registrations, all equally exposed — this was never specific to the executor-binding route it was found on. `URN_MAX_PARAM_LENGTH` (http-limits.ts) derives the new ceiling from the schemas themselves (object-name max length, org UUID length, the one URN shape — `placement` — built from two names) rather than a guessed round number.

### §107. A route-level (`maxParamLength`, malformed-URL) failure bypasses `setErrorHandler`

A route-level (`maxParamLength`, malformed-URL) failure is raised by find-my-way BEFORE Fastify ever dispatches to a route, so it never reaches `setErrorHandler` below — by default Fastify writes its own bare, non-problem+json JSON body (`{error, code, message, statusCode}`) straight to the raw response, which is indistinguishable to a caller from an ad hoc framework bug rather than a documented API error shape. Fastify's `frameworkErrors` constructor option is the one hook that runs early enough to intercept it, and it hands back a real `FastifyRequest`/`FastifyReply` pair, so the fix is to feed the underlying `FastifyError` through the SAME `frameworkClientProblem`/`sendProblem` pair every other framework-raised client error (§7) already goes through — "URN too long" now reads exactly like every other 4xx this API returns, with a `detail` naming the limit instead of a bare 414.

### §109. The SPA fallback had to stop being a REGISTERED `/*` route

THE SPA FALLBACK HAD TO STOP BEING A REGISTERED `/*` ROUTE, or §106/§107's fix is only half true. find-my-way tries a WILDCARD child node with NO length check on its captured segment whenever a more specific match fails for ANY reason — including an over-long `:idOrUrn`. The old `app.get("/*", ...)` registration was exactly such a node, and it was registered for GET only. Measured while writing this fix: a `DELETE` past the new `URN_MAX_PARAM_LENGTH` correctly 414'd (no GET-only wildcard exists to catch a DELETE), but the IDENTICAL over-long segment on a `GET` — the majority verb across the 42-route census in §106 — silently fell through to that wildcard, which called `reply.callNotFound()` and answered with Fastify's bare, non-problem+json 404, indistinguishable from a genuinely wrong path. Any wildcard registered under a prefix that also holds parametric routes has this property, so patching the GET case (e.g. registering the same wildcard for every verb) would not fix it — it would only spread the swallow to every verb.

The fix is architectural, not another guard: `app.setNotFoundHandler(...)` registers its own fallback on a SEPARATE find-my-way router instance (fastify's `four-oh-four.js`) that Fastify only consults once the MAIN router has returned a definitive "no match" — and `onMaxParamLength` on the MAIN router short-circuits that outcome before it ever gets there, for every verb, because there is no longer a competing wildcard NODE on the main router to catch it first. This also means a genuinely wrong `/api/` path now throws `notFound()` (this module's own `errors.ts` helper) instead of calling `reply.callNotFound()`, so it flows through the same `setErrorHandler` as everything else and answers `application/problem+json` too — a related but incidental improvement, pinned in `urn-param-length.integration.test.ts`.
...
### §10. Low-priority catch-all: `setNotFoundHandler`, not a registered route

Low-priority catch-all, registered via `app.setNotFoundHandler`. `@fastify/static`'s exact/static routes above always win over this — it only runs once EVERY real route AND every real asset file has failed to match — so this only ever runs for SPA client-side routes (`/services`, `/graph/abc`, ...) that have no matching file on disk. The explicit `/api/`, `/static/`, `/healthz` guard is belt-and-braces on top of that route precedence, so an unmatched API path still 404s as JSON rather than getting served HTML. See §109 for why this is `setNotFoundHandler` and not a registered `/*` route, which is a correctness requirement, not a style choice.
...
LOAD-BEARING SINCE §8.9 (2026-09-16, `docs/proposals/component-journey-view.md`). Promotion export (route and `exportPromotionBundle`), cosign key custody (`ensureInstanceCosignKey` / `getInstanceCosignPublicKey`, commander or retrans only) and all dependency automation refuse an UNDECLARED deployment fail-closed. So `main.ts` logs ONE warning at boot when this is false (`boot-checks.ts`'s `federationRoleUndeclaredWarning`), and every shipped launcher declares it: Helm always renders it, the compose eval stack, `docker-compose.two-member.yml` and `pnpm dev` set `commander`, and `docker-compose.federation.yml` sets `commander` for domain A and `outpost` for domain B.
...
### §108. `URN_MAX_PARAM_LENGTH` — Fastify's path-parameter ceiling, not a body ceiling

`URN_MAX_PARAM_LENGTH` lives next to the body-size ceilings because it is the same shape of decision (a framework default that was never sized against this API's actual payloads) even though it bounds a PATH SEGMENT, not a body. See its own doc comment for the full numeric derivation (worst case: a `placement` URN, the only URN shape built from two independent object names) and app.ts §106/§107 for where it is wired into the Fastify instance and why an over-long segment needs its own error-shaping hook.
