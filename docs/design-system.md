...
### 1.6a Target/stage row grammar (owner mockups, 2026-09-11; implemented on the wire-available subset only — see `docs/design/mockups/2026-09-11/README.md`)

The recovered mockup session settled a row grammar for a wave target (`PipelineWaveCard.tsx`) that also now applies, where the underlying data exists, to a component-pipeline stage tile:

- **Status is a coloured OUTLINE on the row**, not a filled badge plate. Tones map onto the existing §1.5 palette (`success` emerald / `info` blue / `warning` amber / `danger` red), plus one row-only tone, `idle` — **dashed** `border-slate-300`, for `pending`/unrecognised statuses. Idle must render dashed even where §1.5's own `neutral` badge tone would be a plain solid border elsewhere: a target nothing has touched yet must never share a border style with a real outcome. A hold (freeze/continuous-probe = `warning`, stage-dependency-only = `info`) outranks the raw status — the same precedence the held badge already encoded. The `TargetReticle`/stage mark picks up the row's own tone. The status WORD is kept beside the outline (plain coloured text, not a second filled pill) — an outline alone makes colour the sole carrier of status, which fails colour-blind readers; this was flagged in the owner session and never explicitly declined, so it stays. Implementation: `components/pipeline/wave-status.ts`'s `targetOutlineTone`/`targetOutlineBorder`/`targetReticleTone`/`targetStatusTextTone`; consumed by `PipelineWaveCard.tsx` and `component-pipeline.tsx`'s `stageOutlineTone`.
- **Subtitle: `<Type> · <provider>`, plain text, no glyph** — "type as plain text, no glyph — icon not needed" (owner: the mark above the row already says "this is a target"). `Type` is the ADR-0007 routing Type (`configuration`, `image`, …); `provider` is the executor binding's `pluginModule` (e.g. `argocd`) where the caller has fetched bindings for this target — today only `/changes/{id}/pipeline` (`change-pipeline.tsx`'s `linksFor`). A caller that has not fetched bindings (`change-detail.tsx`, `campaign-detail.tsx` embed `PipelineWaveCard` without `linksFor`) renders the Type alone — never a guessed provider. **Increment-3 gap**: wiring the same per-target binding fetch into those two views would complete this everywhere `PipelineWaveCard` renders.
- **Card proportions: ~610px max, not a hyperwide stretched bar** (`max-w-[610px]` on the wave `Card`) — the owner's own narrowing rounds went 1040px → 700px before landing here.
- **Source tiles state their repo KIND in text**, not a per-`sourceKind` glyph: one generic `Warehouse` mark for every source tile (never a GitHub/Gitea/GitLab-specific icon — `sourceKind` is the git host, not the routing Type), and a kind line derived from the mapping's `pathPattern`, `type` and `journeyKind` (`component-pipeline.tsx`'s `sourceKindText`): a `configuration`-category source reads bare `"config"`; a `build`-category source reads `"<pathPattern> → <type>"` when a path is declared or `"service code → <type>"` for a whole-repo mapping (`journeyKind: "config"` overrides a build-category source back to bare `"config"` — the §8.14 mistyped-source case); an `infrastructure`-category source has no build/config split and always reads `"<pathPattern> → infrastructure"`. The routing Type itself is labelled **"declared, not inferred"** next to its value — the schema comment is emphatic that it cannot be derived from `sourceKind`. `refPattern: null` already read as "any branch" before this round (`pipeline-source-any-branch`); the mockup's "any ref" wording is the same fact under a different word and was left as-is.
- **Registry tiles already carry one generic `Container` mark** per tile (via `NodeShell`/`PipelineIcon`'s `NODE_ICON.registry`) — no change needed.
- **Scan severity + digestMatch are surfaced in the compact (always-visible) Scan & sign tile**, not hidden behind Details — the mockup's `.scan` panel sits structurally between Registry and the first deploy wave, which `laneNodes`' fixed node order (`source → build → registry → scan-sign → source → wave`) already placed the real tile at; the gap was that severity counts and `digestMatch` were previously only in the per-row Details table. **Honesty (journey-view §8.9/§8.13):** `digestMatch === null` (evidence never recorded it) renders as an explicit, amber-worded stated absence — never as either boolean, and never let a `pass` verdict beside it imply "verified against what ships". Implementation: `component-pipeline.tsx`'s `newestScan`/`ScanSeverityCallout`.
- **Approval/fan-in gate chips on the inter-wave connector** already render via `PromotionArrow`'s existing `approval`/`held` states (`arrowInto` in `component-pipeline.tsx`) — but only the DECLARED requirement (`gate.policies[].requireApprovals[].count`) is on the component-pipeline wire, never the live granted-vs-required tally the mockup's `"approval required · N of M"` chip needs. **Increment-3 gap**: an API addition (a live approval count per wave gate) is needed before that literal chip can be built honestly; fan-in counts are the same shape of gap and were explicitly left to a separate API-design effort.
- **Rollout pip-stepper** (`microservice.html`'s `.roll`/`.pips`/`.pip`, pipeline-mockup-data.md §5, increment 2): a labelled `Canary` row of one pip per `observed.rollout.stepCount` — filled blue for the completed/current step, pale slate for the rest, `step N · M%` at the right (`step` is 0-based and shown as `min(step + 1, stepCount)` of `stepCount`). Rendered ONLY when the executor reported `stepCount`; its absence (a blue-green Rollout, an older Rollouts version, a non-ArgoCD executor) keeps the pre-existing plain `phase · step N · weight N%` text — never a guessed or zero-length stepper. A `stale` `observedFreshness` (older than `OBSERVED_WEIGHT_FRESHNESS_MS`, shared with `stage-dependency-hold.ts`) greys every pip from blue to slate rather than dropping the "this is live" signal onto a reading that may no longer be true, prefixes the text with "last seen", and carries the amber-dashed `unknown` badge with the age (§1.6a's status-word rule: colour is never the only signal). A `not_reported` reading (the target executes at another domain instance and no observation has been federated up to this one) renders honest text and no pips at all — never merely "never observed", which would promise a reading might still arrive from this instance's OWN observation loop. Implementation: `PipelineWaveCard.tsx`'s `RolloutStepper`/`formatAgeAgo`.
- **The per-target CHECKS RAIL** (`target-redesign.html`'s `.checks` strip, pipeline-mockup-data.md §3, increment 3): a 2×2 grid of chips at card width, under a dashed top rule, carrying **all four hook kinds, always, in fixed pipeline order** — post-merge → post-deploy → continuous → bake. Fixed position is the point: the third cell is always the canary, so an unbroken column of dashed thirds reads instantly as "nothing here is being watched". Each chip is a dot + kind label + right-aligned value word. **The rule the rail exists to keep — not bound and bound-but-silent must not look alike:** a kind nobody declared renders white/dashed/hollow-dot with an em-dash and a tooltip, and **never amber** (nobody promised it — §1.5's structural-absence rule); a declared probe that has never reported (`no evidence`) or that reported and went quiet (`stale · <age>`) is amber-dashed, because someone promised the check and an operator should chase it. Two greys that differ only by word, deliberately: `not run` (declared, nothing reached it) and the undeclared em-dash — "one is waiting, the other will never happen". Bake's **`10 m — not started`** is a third grey word, rendered rather than omitted: the quiet window has not opened because the target has not deployed. `pass` / `clear` are emerald, `failed` / `aborted` / `alarm` red, `running` / `dispatched` / `baking` blue. A continuous pass carries **both** the word and the evidence age (`pass · 40 s`) — colour is never the only signal. A `state` this bundle does not recognise (a server from a later increment) renders its own wire word amber-dashed with "NOT a pass and NOT a failure", never a tone that reads like a verdict; a `basis: "unresolvable"` rail renders one amber-dashed stated-absence line instead of four em-dashes, which would claim nothing is declared. **Grain travels in the tooltip:** `pipeline_hook_runs` is keyed `(change, hookId, waveIndex)` and not by target, so a post-merge chip says "this is the CHANGE's run" and a post-deploy chip "this is the WAVE's run" — the rail must never read as "this target's post-deploy test" (§3.4). `checks` absent on the payload renders **no rail at all**: absence means this caller does not know, never "nothing is declared". Implementation: `components/pipeline/PipelineChecksRail.tsx`; honesty pinned in `PipelineChecksRail.test.tsx`.
...
## 6. IDIOMS (established this branch — read from the code cited, not from memory)

Each entry: the rule, WHERE it lives, and the mutation that would catch a violation.

1. **Typed-name delete confirm.** A destructive delete is gated on the operator retyping the object's own name exactly — not a generic "type DELETE" — inside a danger-styled dialog that names the type, the name, and the URN. Delete stays disabled until the typed text matches; a server refusal renders its sentence verbatim and leaves the dialog open (no optimistic removal from the list). Lives in `apps/web/src/routes/registry-detail.tsx`'s `DeleteObjectCard`. Mutation: relax the match to case-insensitive or substring, or remove the disabled-until-match guard — a confirm-dialog test should go red either way.

2. **The governed-here line.** A line asserting "moves here are governed" renders ONLY off a successful, positive read (`enforced === true`); pending, error, and a successful `enforced === false` all render nothing — silence is not itself a claim, so there is nothing dishonest to say for those three states short of adding a negative-claim line, which this idiom deliberately does not do. Lives in `apps/web/src/routes/registry-detail.tsx` (`GovernedHereLine` / `GovernedHereLineForObject`). Mutation: render the line for a pending or `enforced: false` query — a test pinning "renders nothing while pending" / "renders nothing when not enforced" goes red.

3. **Dry-run-preview-gated write.** A write whose blast radius cannot be read off the request (here: every component subscribed to a coordinate) runs `dryRun: true` first and shows the preview as a report; the real write button is enabled ONLY while a preview exists for the SAME inputs — editing any field after a preview invalidates it and re-disables the button. Lives in `apps/web/src/routes/admin-dependencies.tsx`'s `DeclareDialogBody` (`previewCurrent`, the `key` computed from ecosystem/coordinate/producer). Mutation: change one field after a successful preview and assert the write button is still enabled — should go red; or drop the `previewKey` comparison so a stale preview stays "current".

4. **The unnamed pill.** A server-enriched name field that comes back empty (`""`) renders the bare id in monospace PLUS an amber `Badge variant="unknown"` reading "unnamed", whose tooltip states the server fact — a dangling reference, not a loading state or a client-side failure. Every name-bearing cell in the same row follows the identical rule; one cell may never flag the case while a sibling cell silently prints a bare id. Lives in `apps/web/src/routes/admin-dependencies.tsx` (producer cell and declared-by cell, `producer-unnamed` / `producer-declarer-unnamed`). Mutation: feed a fixture with one empty-name field and assert the "unnamed" badge is absent, or assert a sibling cell in the same row prints a bare id with no badge — either should go red.

5. **Names-from-the-wire.** The server enriches list/detail responses with `{objectId, name}` pairs; the client renders that `name` directly and never issues a follow-up lookup to resolve a name from a bare id (no N+1), and never renders a bare id where the response carries a name field (an empty name is the "unnamed pill" case above, not a fallback to look elsewhere). Lives wherever a response is documented as carrying enriched names — e.g. `apps/web/src/routes/admin-dependencies.tsx`'s module header ("every name rendered … is READ off the server's enriched response … never looked up N+1"). Mutation: point a name-bearing cell at a client-side lookup keyed by id instead of the response field — a mocked-SDK test that never wires the lookup's data source should go red (nothing renders, or renders undefined) where today it renders the name from the fetch mock alone.

6. **The consequences dialog stays open on its report.** A write whose real-world side effects outlive the write itself (here: `openBumpAuthorships[]` — pull requests SCP opened elsewhere and does not close on retraction) renders that report as the dialog's content and does NOT auto-close on success; the operator dismisses it explicitly, because the report — not a toast — is the take-away. Lives in `apps/web/src/routes/admin-dependencies.tsx` (the retract dialog's `openBumpAuthorships` rendering). Mutation: auto-close the dialog on a successful retract response — a test asserting the report is still visible after the write resolves should go red.

7. **The truncation pill (forward pointer — landing with M23.1g).** `docs/proposals/observed-truncation-ui.md` §3 sets the general form of rule 4 above for OBSERVED-STATE fields specifically: an amber `Badge unknown` pill appears exactly where a rendered claim would otherwise be false (a dropped rollout report, a wholly-elided image list, a wholly-elided observed state) — never for a cut that leaves the rendered claim true, which stays tooltip-only (a trimmed image tail). This entry is a pointer, not a landed rule: the proposal is the source of truth until the M23.1g render ships (`apps/web/src/components/pipeline/PipelineWaveCard.tsx`), at which point its concrete `data-testid`s and helper (`truncatedBadge(truncation, field)`) belong in this section beside entry 4.