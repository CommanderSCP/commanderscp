...
- **Done / verified by (sketch — per-increment DoDs in the proposal's Phasing table; nothing here is complete):**
  ...
  - **13.3a** the promotion scan step + scanner registry: an ephemeral runner at the commander scans a subject artifact (container image + a `trivy vm` machine-image case) pulled by digest over the allowlisted channel → evidence parses via `ScanEvidenceSchema`, lands **commander-resident** → unmodified M17.5/E6 machinery consumes it (a pipeline-less promotion scans → evaluates → signs → exports, zero gate-code changes); valid org-pipeline evidence short-circuits the managed run (both ingresses proven); scanner selection follows registry rows by artifact type (unassigned type + no evidence still refuses at E6); scanners exist only in the runner image. **13.3b** scanner DB (+ SCAP content) rides the relay/bundle path as a `blob`, digest-verified at the receiver; E6 never evaluates it as a substantive artifact (vacuous exemption, not evidence — documented); runner proven offline; refresh runbook merged. **13.3b-ii (DONE):** the runner's offline DB **pre-load seam** (`SCP_SCAN_DB_DIR`, a second `docker cp`; unset ⇒ the baked fallback) + the server-maintained DB **cache** (`SCP_MANAGED_SCAN_DB_CACHE`, an optional Helm PVC) the commander pre-loads from; a **connected** operator refresh (`scp scan-db refresh` — allowlisted skopeo-pull, atomic swap, schema-compat assertion vs the pinned Trivy) and the **air-gap operator-load** (`scp scan-db load` — a cosign-signed DB blob walked across the CDS, digest-bound + detached-signature verified before accept; the commander has **no** into-commander byte channel, so "promotion-channel refresh (air-gapped)" in the proposal is **corrected** to operator-loaded, owner decision 2026-07-24); a commander-level, instance-scoped **staleness policy** (soft/hard max-age, operator-configurable API→SDK→CLI at runtime, modeled on M17.5's `scan_requirement_floors`; missing/corrupt/hard-stale fail closed, soft-stale warns — surfaced on the `ScanEvidence` + Decision); the trivy-db/SSG asymmetry (SSG stays baked — no OCI upstream). Tests: pre-loaded-DB scan produces valid digest-bound evidence offline; missing/empty cache fails closed → E6 refuses with a `decision_id`; stale-past-hard fails closed, warn-age scans + warns; operator-load refuses a tampered/wrong-key blob with no cache write.
