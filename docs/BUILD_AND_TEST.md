...
- **Done / verified by:**
  ...
  - **Multi-region Argo CD (M15.6):** an integration test binds **two** regional Argo CDs (AMER + APAC deploy-targets) to one prod env through the config surface and proves each target resolves to its own binding (`getExecutorBinding` per target); the round-trip goes only through the generated SDK (no bypass).
