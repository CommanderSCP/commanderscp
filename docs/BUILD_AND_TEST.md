...
- **Contents:**
  ...
  - **M17.4 Cross-hop cosign verify — two distinct steps** ([ADR-0015 §6](adr/0015-cosign-cross-boundary-signing.md)), because federation bundles are **metadata-only** (ADR-0009) so artifact bytes are absent at import:
    ...
    - **(b) Per-artifact `cosign verify` where the bytes land (DEFERRED → M15.5)** — at the outpost's local Gitea/registry, against each artifact's **origin** `signatureRef`. **Blocked on the unresolved artifact-bytes transport channel** (ADR-0015 §6b, `docs/proposals/outpost-local-artifact-infra.md` — "the honest open gap"): a federation bundle is metadata-only, so the bytes are absent at import. **(a) is metadata-only and complete without them**; (b) is cleanly deferred (a `TODO` pointer to M15.5 lives at the gate), not half-built.

    Lands the **outpost-side** verify (ties [ADR-0011](adr/0011-universal-outpost-validation.md), M15.2 — the outpost's pre-deploy validation IS this same import gate, one implementation) and, where retrans exists, the CDS-boundary verify (retrans is naming-only/unbuilt; retrans-as-a-real-component is its own work).
