...
**Runner-image builds in the integration job use the LEGACY builder (`DOCKER_BUILDKIT=0`).** The integration job (stage 5) runs on a Docker-in-Docker (DinD) runner and does two things in the same daemon: it docker-BUILDS the ephemeral runner images (`scp-runner-scan` for the promotion scan step, `scp-runner-iac` for managed-IaC) and, via the managed-scan/managed-iac plugins, CREATES `--network none` runner containers. Modern Docker's default BuildKit builder opens a persistent embedded gRPC "session"; inside the homelab DinD that session deadlocks against the concurrent/subsequent net=none container operations (`session healthcheck failed fatally: Unavailable: ... only one connection allowed`), hanging every integration run on `main` and all PRs. The fix is to build those runner images with the legacy builder by passing `DOCKER_BUILDKIT=0` in the `docker build` child env of the integration tests (`apps/server/src/federation/promotion-scan-step.integration.test.ts`, `packages/plugins/managed-iac/src/managed-iac.integration.test.ts`). Both runner Dockerfiles are plain single-stage `FROM`+`RUN`+`COPY` builds with **no** BuildKit-only features (no `RUN --mount`, `--secret`, or heredocs), so the legacy builder produces functionally identical images. Do NOT re-enable BuildKit for these builds without first re-solving the DinD session wedge (e.g. a dedicated `buildx` builder instance, or serializing build-then-run so the session fully closes before any net=none container is created).
