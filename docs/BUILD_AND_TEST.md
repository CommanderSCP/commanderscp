...
| 5 | Integration | `pnpm test:integration -- --shard=i/2` — 2-way sharded matrix; per-worker template-DB parallelism; pre-built runner images pulled from GHCR | yes (via the `integration` aggregation gate) |
...
**Lever 1 — pre-built runner images, pulled not built.** The integration tests need two ephemeral runner images: `scp-runner-scan` (promotion scan step) and `scp-runner-iac` (managed-IaC). They used to `docker build` them in-process on *every* run. Now a dedicated `runner-images` CI job builds each image **once per content change** and pushes it to GHCR under a content-hash tag (`scripts/runner-image-tags.sh` is the single source of truth for the tag formula — `scp-runner-scan` hashed over `apps/runner-scan/**` + `tools/trivy/pin.env` + `tools/openscap/pin.env`, `scp-runner-iac` over `apps/runner-iac/**`), rebuilding only on a cache miss (`docker pull || build+push`, mirroring the `ci-image` job). The integration job then `docker pull`s both and exports their refs into `$GITHUB_ENV`; the shared helper `resolveRunnerImage` (`@scp/plugin-testkit`) returns the pre-pulled ref instead of building. With the ref env vars unset (local dev), it falls back to the legacy-builder build — behavior identical to before. `turbo.json`'s `test:integration` task declares those refs in `passThroughEnv` because Turborepo 2.x runs tasks in strict env mode. The manual `publish-images.yml` release workflow publishes `scp-runner-scan` too (alongside `scpd` + `scp-runner-iac`), under the deliberate `sha-<commit>`/`latest` release tags a consuming gitops repo pins against.

**Lever 2 — sharded matrix.** The integration job is a 2-way matrix (`integration-shard`, `shard: [1, 2]`) running `pnpm test:integration -- --shard=i/2`. vitest shards at file granularity, so the two shards cover every file exactly once (verified: 31 + 31 = 62 files, 268 + 275 = 543 tests). The `@scp/airgap` `install.sh` tamper-rejection suite is **not** part of the sharded vitest task graph, so it runs **exactly once** (shard 1 only, via `if: matrix.shard == 1`) rather than being duplicated. A small `integration` aggregation-gate job `needs:` the matrix and goes red unless every shard succeeded, giving branch protection **one stable required-check name** to reference. **Repo-admin action:** update the required status check in branch protection from the old single `5. Integration …` job name to `5z. Integration (aggregation gate)`.
...
### M12 — Service/component model & coupled pipelines
*(Design: `docs/proposals/service-component-model.md` (P1–P4A), `docs/proposals/coupled-pipelines.md` (P4B), `docs/proposals/organize-after.md` (P5); ADR-0005 create-strict, ADR-0006 fail-closed on missing binding, ADR-0007 binding Type taxonomy.)*
- **Goal:** make the service→component graph load-bearing for coordination (every component `part_of` a service, import-permissive/create-strict; executor bindings purpose/Type-scoped per target) and close the coupled-release ordering hole: a change declares `provides: string[]` / `requires: {key, at}[]`; an unsatisfied requirer parks in a `waiting` state and releases on the provider's **execution success** (`validating|promoted` — owner Q1: run successfully, not a human gate). The commander is the single coupling evaluation point: promotion **strips `requires`** on import with a 'requires satisfied upstream' Decision (owner Q2), preserves `provides`; rollback stays org-local and neither waits nor inherits coupling.
- **Shipped:** P1 `service contains component`; P2 containment traversal; P3 purpose-scoped executor bindings; P4A routing Type (ADR-0007); **P4B** — typed `POST /change-sources/{sourceKind}/report` ingress (generated SDK op + `scp change-source report`, incl. `--provides/--requires`), the two Phase-2 security closes (`change` engine-managed on the generic route; `POST /changes` target-authority binding), `waiting` + 3 edges, guarded `advanceWaitingChanges` sweep (fail-closed on malformed `requires`, round-robin fairness, M17.4b artifact gate on release), propose-time `at` resolution (404 on typo), explain/CLI/UI wait surfaces, deterministic `source_mappings` tie-break, promotion `requires`-strip + Decision; P4C cascading rollback deliberately NOT built (named future work); P5 create-strict + organize-after.
- **Done / verified by:** `coupling.integration.test.ts` (park / release-on-provider-success / `at`-scope isolation / no-wait path unchanged / bad-`at` 404 / malformed-requires parks / starvation round-robin), `transitions.integration.test.ts` (seed == code edge table, both directions), `change-authority.integration.test.ts` (both security holes), the federation strip round-trip (no-requires on import, `provides` preserved, strip Decision), `transitions.test.ts` (16-edge machine).
...
### M14 — Federation live-sync substrate + Optional Poke-Mode (per-outpost)
*(Provisional number — post-M11 federation track; sequences after the air-gap CDS work. Design: [ADR-0009](adr/0009-optional-poke-mode-federation.md), `docs/proposals/outpost-poke.md`. Owner **full-scope M14** decision 2026-07-24: build the deferred live-sync SUBSTRATE first — the foundation the poke optimizes — then the poke increments on top.)*
- **Goal:** let a **reachable** outpost (or retrans) stop constantly polling. The commander sends a **contentless** "poke" ("something is pending, come pull") and the outpost pulls over its existing outbound path — no data ever flows downward. **Off by default, configured per outpost.** Builds on M6 federation; the data-direction invariant is unchanged (DESIGN §13 / ADR-0009 restatement: *no data commander→outpost; a contentless wake signal is permitted only where poke-mode is enabled and topology/accreditation allow it*). Regulated partitions (commercial cannot dial into GovCloud), CDS, and air-gap outposts stay pure-pull/bundle. **Grounding finding (2026-07-24, recorded in the proposal + ADR-0009):** the "frequent poll" the poke design assumed already existed did **not** — M6 shipped the `.scpbundle` FILE transport + the `federation-https` PLUGIN contract, but the SCHEDULED HTTP live pull and the outbound mTLS client-cert injection were **deferred** (the M6 PR body + `federation-https` module header flagged this; M8 then built the client-cert *presentation* for the plugin subprocess). M14.0 builds the missing scheduled live-sync substrate; there is nothing to "disable" in poke-mode until it exists. **Reliability model DECIDED** (proposal §4, owner 2026-07-18): sparse safety-net reconcile + pull-on-(re)connect/startup — never poke-only. **Transport DECIDED** (ADR-0009): reuse the ADR-0001 mTLS federation routes, no dedicated listener.
- **Contents:**
  - **M14.0 Live-sync substrate (the M6-deferred HTTP live-sync, built now)** — two coupled pieces, no new public API surface (internal loop + outbound dialer; `/v1` untouched → oasdiff-trivially-additive):
    - **Per-peer mTLS OUTBOUND dialer** (`federation/federation-outbound.ts`) — this instance's enrolled per-domain CLIENT cert/key (the operator-provisioned, deployment-level `SCP_FEDERATION_MTLS_CERT_FILE`/`_KEY_FILE` material M8 established, SAN URI `urn:scp:domain:<ownDomainId>` — **no new CA scheme**) presented on the outbound dial so the commander's `enforceFederationMtls` (ADR-0001) recognizes the caller. **STRICTLY FAIL-CLOSED:** a peer that requires mTLS (an `https://` baseUrl — in this system https ⟺ client-cert-verified) with no usable client cert → the dial **REFUSES** (block Decision), never a silent plain-HTTP/bearer-only fallback. Plain-`http://` peers keep the bearer-only path (backward-compatible; mTLS opt-in per the `federationServerMtls` posture). Certs/keys never logged/argv'd; the per-dial agent is torn down after use.
    - **Outpost live-pull scheduler** (`federation/federation-sync.ts`, `startFederationSyncLoop`) — a pg-boss self-rescheduling singleton mirroring `startInboxLoop`/`startObserveLoop` EXACTLY: DEFAULT-OFF (`SCP_FEDERATION_SYNC_LOOP=1`), interval `SCP_FEDERATION_SYNC_INTERVAL_SECONDS` (default 60s, 5s floor), wired in `main.ts` under `SCP_ROLE=all|worker`. Per commander-peer: POST `/federation/exports` over the dialer, feed the returned `.scpbundle` VERBATIM to the **UNCHANGED** `importSyncBundle` (checksum + Ed25519 at the sequence-anchored key window + hash-chain continuity), advance the cursor. Includes both poke-design backstop legs: **pull-on-startup** (immediate first tick) and **the sparse safety-net** (the interval tick — in poke-mode M14.4 its frequent leg is later disabled while startup + a sparse interval remain). Idempotent + fail-closed on a bad bundle (409 → block Decision, sweep continues); no existing import verification weakened.
  ...
  - **M14.2 Poke endpoint** — a contentless, enrolled-commander-mTLS-authenticated (ADR-0001), rate-limited endpoint on the outpost/retrans that wakes an immediate pull; no request body is trusted; idempotent; **fail-closed without transport identity** (a caller lacking the verified commander mTLS peer identity is rejected, never woken).
  - **M14.3 Commander poke sender** — on a new pending-transfer for a poke-mode outpost (outbox-derived, DESIGN §5), poke that outpost.
  - **M14.4 Scheduler mode + retrans hop chain** — in poke-mode, disable the frequent interval poll but retain the M14.0 **sparse safety-net reconcile + pull-on-(re)connect/startup** so a dropped poke self-heals within a bounded window; propagate the poke across the retrans chain (commander → low-side retrans → CDS → high-side retrans → pokes the outpost locally, ADR-0009).
- **Done / verified by:**
  - **M14.0 Integration (real Postgres + a real HTTPS mTLS listener):** a commander+outpost TWO-DOMAIN round trip where the OUTPOST's pull scheduler pulls+imports a sync bundle from the commander **over mTLS** (test CA + per-domain client/server certs; the commander's `enforceFederationMtls` accepts the outpost's enrolled identity; the imported bundle verifies Ed25519 as today); **fail-closed proven** (an mTLS-required peer with no client cert → dial refused, block Decision, no plain-HTTP fallback); the loop is default-off (no env → no tick) and pull-on-startup fires. **Unit:** cert resolution + fail-closed path; loop self-reschedule + interval + opt-in.
  - **M14.1–M14.4 Integration (real Postgres):** a poke triggers an immediate pull; poke-mode disables the frequent poll; a **non-commander caller is rejected**; a replayed/contentless poke is idempotent; **a dropped poke self-heals via the safety-net backstop**; poll-mode outposts and the air-gap/bundle path are unchanged.
  - **Parity:** `federation.pokeMode` is set/read only through the generated SDK (no bypass); oasdiff-additive within `/v1`.

### M15 — Per-Outpost Local Artifact & Source Infrastructure (Gitea; no local Harbor) — **COMPLETE (2026-07-22: M15.5(c) was the last open item)**
...
- **Contents (phased):**
  ...
  - **M15.5 The artifact byte channel** ([ADR-0019](adr/0019-artifact-byte-channel.md); reframed from "optional/later … if operator-loaded bytes prove insufficient" by the owner's finish-all-of-M15 mandate, 2026-07-20) — per-tier artifact-bytes transport, three parts:
    ...
    - **M15.5(c) Retrans byte-relay — DONE** (`apps/server/src/federation/retrans-relay.ts`; API `POST /federation/relay` + `/federation/relay/import`, CLI `scp federation relay`/`relay-import`, config + runbook `docs/runbooks/retrans-relay.md`; the DoD suite is `apps/server/src/federation/retrans-relay.integration.test.ts` — three isolated federation domains (commander/retrans/outpost), two `registry:2` containers plus an htpasswd-authed third, real cosign + skopeo: the full round-trip passes the receiving M17.4(a)+(b) gates end-to-end, a wrong-key artifact refuses with a `retrans-relay-validate` block Decision + audit and never reaches the destination, a transit-tampered tarball refuses at the destination with nothing pushed, a non-allowlisted source host is refused before any dial, a non-retrans role refuses to relay, and the vaulted per-registry push credential works against the authed registry without ever appearing in logs/Decisions/audit). **This completes M15 and M17.** The [ADR-0004](adr/0004-service-naming-commander-outpost-retrans.md) validate-then-relay made real (ADR-0019 §2): resolve the promotion manifest's authorized digests → skopeo-pull bytes from the source registry → **VALIDATE** with the M17.4 digest-bound machinery (`verifyAuthorizedArtifactSet` — a failing/tampered/unauthorized artifact **never crosses**) → package a signed OCI-layout tarball (build-bundle machinery) → relay across the CDS as a file (`.scpbundle` pattern) → push into the destination local Gitea **by digest + re-inspect** (install.sh pattern) → the receiving outpost **still** runs M17.4(a)+(b) (defense in depth — zero trust in the relay). Credentials are the ADR-0019 §3 **artifact-store class**: vaulted (`secrets` table), scoped per-peer/per-registry, source-READ + destination-PUSH only — registry creds, not exec-infra creds. **DoD:** a **two-registry Testcontainers round-trip** — pull from the source registry, validate, tarball, push to the destination registry, and the receiving M17.4 verify passes; a **tampered/unauthorized artifact never crosses** (block Decision + audit event at the retrans); the credential class is vaulted + scoped (no admin/delete grants); and the `SCP_ARTIFACT_OCI_REGISTRY_HOSTS` allowlist (ADR-0019 §4 — closing the #108 residual; unset = fail-closed) is enforced on the OCI verify/pull path (**DONE — verify path** (foundation 2): the location's registry `host[:port]` must match an allowlist entry exactly BEFORE cosign is invoked — a disallowed host fails the artifact without a dial, unset refuses every OCI verify, symmetric with `SCP_ARTIFACT_BLOB_BASE_URLS`; `federation/artifact-verify.ts` + pre-deploy-gate suite axes (i)/(j); the relay's skopeo-pull side lands with the relay); and a **pinned skopeo is vendored into the runtime image + air-gap bundle** (the M17.3 cosign-vendoring pattern — today skopeo is an operator-`PATH` prerequisite, so the relay's server-side pull/push steps require this net-new vendoring).
...
- **Done / verified by:**
  ...
  - **Retrans byte-relay (M15.5(c)) — DONE:** `apps/server/src/federation/retrans-relay.integration.test.ts` (three isolated domains + two `registry:2` containers + an htpasswd-authed third + real cosign/skopeo) proves the DoD: full round-trip with the receiving M17.4(a)+(b) gates passing end-to-end; tamper never crosses (source-side wrong-key ⇒ `retrans-relay-validate` block Decision + audit, destination empty of the digest; transit-tampered tarball ⇒ destination refusal, nothing pushed); OCI-host allowlist refuses a non-allowlisted source before any dial; a non-retrans role refuses to relay; the vaulted per-registry artifact-store credential pushes to the authed registry and never appears in logs/Decisions/audit.
...
### M17 — Supply-Chain Gate (scan + SBOM + cosign signing + cross-hop verify + scoped scan policies) — **COMPLETE (2026-07-22: the M15.5(c) retrans relay closed the last cross-reference — the CDS-boundary verify hop is real)**
