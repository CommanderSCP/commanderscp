...
THE OBSERVED CI RUN a change names — component-journey-view.md §3 Segment 2's "upstream build" case. `sourceKind` is the change's own `source_kind` ("github"/"gitea"/"gitlab" — the only kinds `observed-run-facts.ts` reads run identity out of today). `repo` and `url` are nullable because the "carries run identity" predicate accepts either alone (a citable run id plus AT LEAST ONE of url/repo); `workflowName`/`workflowPath` are nullable because not every provider's writer shape cites them (gitea's `GiteaActionRun` and gitlab's `GitlabPipeline` name neither). `observedAt` is when SCP recorded the RUN EVENT (`change_source_events.created_at`), not a field read out of the run payload — every writer names a run creation time under a different, unpinned key. `changeId` is the RELEASE whose commit the run was for. Since 2026-09-16 a run is a stored event, never a change (docs/proposals/run-events-are-not-releases.md), so this is no longer "the change the run was read off".
