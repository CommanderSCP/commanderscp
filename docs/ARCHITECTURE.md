...
Three views of the same system. Every instance — parent or child — runs the identical `scpd` image plus PostgreSQL; the differences are configuration (role, enrollment) and which objects the instance is the single-writer authority for.

## 1. Parent instance (detailed)

![Parent instance](diagrams/parent-instance.svg)

The parent is the Global Coordination Layer (DESIGN.md §13). What this view shows:
...
- **Federation Engine, parent side** — builds the outbox-derived, hash-chained sync journal; signs segments and approval attestations with the parent's Ed25519 domain key; serves the journal endpoint children pull over mTLS; exports/imports `.scpbundle` files for air-gapped children.
- **Authoritative global config** — domain registry, org structure, global policies, release topologies, campaigns/initiatives, control definitions. The parent is the single-writer origin; every child holds these as read-only replicas.
...
- **Cross-domain view** — children report status upward; the parent UI shows every domain, its sync freshness, in-flight changes, and campaign roll-up. The parent never edits child-owned data.

## 2. Child domain instance (detailed)

![Child instance](diagrams/child-instance.svg)

A child (Commercial, GovCloud, air-gapped, …) is where changes actually get coordinated against execution systems (DESIGN.md §9–§13). What this view shows:

- **Federation inbound** — parent config arrives by mTLS journal pull, `.scpbundle` file, or operator hand-entry (`provenance: manual`, reconciled on the next signed import). Every import verifies Ed25519 signatures, the hash chain, and approval attestations before applying.
...
- **Federation outbound** — local status, changes, and audit segments signed with the child's key and returned to the parent by mTLS or bundle file.
...
- **Parent at the hub** — global config flows down as read-only replicas; status and audit flow up; the air-gapped spoke exchanges the same signed artifacts as files over removable media.
- **Three children, one machinery** — connected Commercial, stricter GovCloud/FedRAMP, and a fully disconnected air-gapped domain all run the same image with the same engines; only policy strictness and transport differ. Every domain is fully operational when disconnected.
