...
The three are inseparable from the row, so they live WITH the row rather than beside each caller. `iac/plans-repo.ts` and the route call the same two functions; neither reimplements any of it.
