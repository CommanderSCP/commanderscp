...
**poll-events-carry-ref (fix/poll-events-carry-ref).** The commits LIST response carries no ref per commit either, and `pollCommits` used to omit `sha` from the request entirely — which listed whatever branch GitHub currently treats as the default and stamped no ref on the resulting events. `correlation.ts`'s ref-scoped `source_mappings` rows fail closed on any event with no ref, so the moment an operator configured one, every polled push silently stopped matching it — with no error anywhere, on the poll path that is how the homelab estate ingests almost all of its coordination. The fix resolves the repo's default branch first (`GET /repos/{owner}/{repo}`.`default_branch`), passes it back as the list's own `sha` parameter, and stamps the SAME branch as `refs/heads/<branch>` on every event — true by construction, not an assumption about what the API defaults to. A resolution failure (network, 404, missing field) falls back to the pre-fix behaviour: list the implicit default, emit no ref. `pollRuns` gets the equivalent fix for free — GitHub reports `head_branch` directly on each run resource, so no extra resolution call is needed there. The gitea and gitlab adapters carry the identical two fixes (gitea's commits/runs endpoints accept the same `sha`/`head_branch` shape; gitlab resolves `GET /projects/:id`.`default_branch`, passes `ref_name`, and disambiguates a pipeline's `ref` against its own `tag` boolean rather than guessing branch vs. tag).
