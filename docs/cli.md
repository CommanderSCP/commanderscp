...
Relationships have one (`DELETE /relationships/{id}`), and it works even when an endpoint is dead. (⚠️ The paragraph below is the original ruling and is kept for its reasoning; two of its claims have since been measured false — see the 2026-09-19 entries and §69a.) The projection rows do NOT have an id-addressed door — `deleteMapping` matches on the identity TUPLE and the binding door addresses its target object — so repairing them from this report's `id` alone is not possible today. Rather than reach past the API into SQL, this command repairs the edges and NAMES the rest, with the count, so the output can never read as "all clean" when it is not.
...
2026-09-19, LATER THE SAME DAY — **`--repair` NOW REMOVES ORPHANED EXECUTOR BINDINGS, AND THAT MEANS IT REMOVES EXECUTION ROUTES.** This round recommended edges-only, on the reasoning that deleting a binding is a larger consent question than deleting an edge whose endpoint is already gone; **the owner overrode that and chose full repair, with no prompt and no confirmation flag.** The rejected recommendation is recorded beside the ruling on purpose, so the next reader does not have to rediscover that the narrower option was considered.

⚠️ WHAT AN OPERATOR IS ACTUALLY AGREEING TO by typing `--repair`: an orphaned `executor_bindings` row carries a target's `plugin_module`, `config`, `secret_refs`, `allowed_hosts` and `external_ref`. Removing it detaches that pipeline **permanently** — the table has no `deleted_at`, so there is no tombstone and nothing to restore from. Two things bound the blast radius, and both are structural rather than advisory: only rows the report already lists are touched, and a row is listed only when its **target object is already soft-deleted**, so a live target's binding can never be reached; and the loop calls `executors.deleteBinding`, the very door `scp executor unbind` uses, never a bulk `DELETE`.

**THE RECORD IS THE MITIGATION, because there is no prompt.** Two independent records, deliberately. Every removal writes its own `executor.binding.delete` event to the hash-chained audit log in the same transaction as the delete (charter principle 6); AND the command prints **one line per removed route** — `deleted executor binding <type>/<lane> on '<name>' (<urn>)` — after the totals, so a repair run is reconstructable from its own output without anyone going to the audit log. A count alone is not a record: it cannot answer "which pipeline did this detach?", the only question that matters the morning after an unprompted repair. There is no Decision record on this path, and that is a choice rather than an omission — `executor.binding.delete` is an operator action, not an engine verdict, and minting a Decision only when the same removal arrives via `--repair` would make repair diverge from the verb it exists to reuse.

**THE `repairable` COLUMN NOW MEANS ONE THING FOR EVERY KIND: "would `--repair` act on this row?"** It already meant that for an edge (`false` for a replica). It said `true` for all three projection kinds, which was **already untrue** — `--repair` never touched them and the summary line said as much two rows below — and the moment `repairable` became the filter an operator selects the actionable set with, a decorative `true` became a trap. Now: `false` for a source mapping (its door matches a five-part identity tuple this report does not carry), SERVER-COMPUTED for an orphan placement (§69a — a placement does have a door, and the server is the only thing that knows whether it would refuse), and `!policyManaged` for a binding — ⚠️ this last one moved OFF the CLI and onto the server the same day; see §69b, where "computed here" stopped being true for any of the four kinds. One predicate, `repairable == true`, selects exactly what a repair run will take — and `type` / `lane` are printed beside it so the per-row route can be driven without parsing `detail`.

**POLICY-MANAGED BINDINGS ARE SKIPPED, with the reason printed.** `policyManaged` is read off `managed_by_policy_id` and carried as a STRUCTURED field on the response (`OrphanExecutorBindingRow`), never inferred from the detail text — as are `targetType` and `lane`, which `--repair` has to pass and which a mis-parse of `detail` would silently turn into `build`. A managed row is re-derived from the live placements every reconcile tick and reaped by `pruneUnwanted` once its target is a tombstone, so repairing it here would at best race the reconciler. Same shape as `DanglingRelationship.repairable: false` for a replica edge: reported, never attempted, and the output says how many and why. Source mappings are still left alone — a mapping is addressed by a five-part tuple this report does not carry. **Orphan placements are NOT: see §69a, written the same day, which measured the "no door at all" claim false and has `--repair` withdraw them.**

### §69a. `--repair` now also withdraws orphan PLACEMENTS, and prints each one

2026-09-19. The sentence above — "the projection rows do NOT have an id-addressed door" — was true of two of the three arms and **false of the third**, and the false third is the one printed last, where a reader stops. A placement is not a projection row at all: it is a graph object with its own id, its own URN and its own door, `DELETE /api/v1/placements/{idOrUrn}`, reached from the CLI as `scp placement withdraw`. Measured, not assumed — `routes/placement-orphan-delete.integration.test.ts` walks that door against a placement whose component was tombstoned under it, by id and by URN, and it answers 200 both ways. So `--repair` repairing the edges and then counting the placements under "no id-addressed door" was printing a receipt that told the operator to go and do by hand the one thing the command could already have done.

The consent question §69 raises about bindings does not transfer. Deleting a binding destroys an execution route and its secret references **hard**, with no tombstone to restore from; withdrawing a placement is a SOFT delete of an object whose two endpoints are already gone, and the object, its revision and its `placement.delete` audit event all survive it.

WHAT IT PRINTS, AND WHY EACH ROW GETS ITS OWN LINE. `placements-deleted <n>` followed by one `placement-deleted <urn> (<deadEnd> dead)` per row, and `placements-skipped <n>` followed by one `placement-skipped <urn>: <reason>` per row. A receipt that said "3 deleted" would leave the operator unable to name which three, and a placement's URN is the only handle there is to re-declare it — the object is gone, both endpoints are gone, and nothing else in the output names it.

WHAT IT SKIPS IS DECIDED BY THE SERVER, NOT HERE. `repairable` and `blockedReason` come off `/graph/integrity` (docs/schemas.md §296b); the CLI neither computes nor overrides them. That matters because the CLI has now had this column wrong for this arm in BOTH directions — the literal `true` (wrong for a replica, for a placement an executor binding still holds, and for a stack-managed row, all three of which `--repair` would have 409'd on) and then the literal `false` (wrong for every other placement). A constant cannot be right here: only the server can see `origin_domain_id`, `managed_by_stack`, and whether a binding still names the row. A skipped row always prints its reason, because a `skipped: 3` with no reason is the line that gets read as "nothing to do here".

ORDERING: BINDINGS BEFORE PLACEMENTS, and it is load-bearing now that §69 has `--repair` delete bindings too. A binding on a placement is what makes that placement unrepairable (orphan-guard route 6 refuses the tombstone), so reversing the two would guarantee a 409 in any run where the populations overlap. It cannot help WITHIN a single run either way — `repairable` was computed server-side before either loop started — so an overlapping estate clears in two runs, and the second run's report says so rather than the first run half-failing.

THE `detail` COLUMN CARRIES `[blocked: …]`. A blocked row's reason is in the report itself, not only in `--repair`'s output, so `scp graph integrity` on its own already tells the operator which command unblocks each row.

### §69b. `repairable` stops being computed here at all — the CLI only READS it now

2026-09-19, LATER STILL. §69's own "Now: … `!policyManaged` for a binding" was true when written and became the exact bug §69a had just fixed for placements: a rule this file computed from a raw field (`policyManaged`), duplicating a verdict a non-CLI caller of `/graph/integrity` had no way to reach except by reimplementing that same rule. §296c (docs/schemas.md) moved `repairable`/`blockedReason` onto every arm's row type, server-computed, so this file's job shrank to exactly what it already was for placements: read the column, print it, filter on it.

Three call sites changed, all in the same direction — from `!r.policyManaged` (or a hardcoded `false`) to `r.repairable`:

```text
the non---repair table view's `repairable` field  -> reads r.repairable for all three kinds now
--repair's bindingsToRepair filter                -> report.orphanExecutorBindings.filter(b => b.repairable)
--repair's per-row skip line                       -> names b.blockedReason, the same shape §69a's
                                                       placement-skipped line already used
```
