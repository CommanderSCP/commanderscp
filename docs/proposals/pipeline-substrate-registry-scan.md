...
- **Wire**: `stages[].outpost` and `unplacedStages[].outpost`: `{ state: "outpost" | "self" | "peer-without-outpost" | "unknown-domain", id, name, trustTier, peerDomainId } ` (nullable identity fields per state). `self` = the target's origin is this instance; `peer-without-outpost` = origin is a paired peer with no outpost object registered (name = the peer's name); `unknown-domain` = origin names no peer known here. Batched: one query over live `outpost` objects + the peers list.
- **Tile**: an "Outpost" line in the compact part of every target tile: `outpost field-outpost · il5` (link to `/federation/outposts/$peerDomainId` on the commander site only), `this instance (hq-commander)`, `peer prod-highside — no outpost record` (quiet, title explains), or `origin domain not known here`. Never derived from the target's name or containment `domain_id` (GLOSSARY: containment has nothing to do with deployment topology).
