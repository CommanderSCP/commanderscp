...
- **Migration**: built-in relationship type `publishes_to` — from `component` to `execution-system`, cardinality `many_to_one`?—**no: `many_to_many`** is not the intent either; use the same shape as `deploys_to`'s cardinality (read 0002:189 and match), `is_builtin=true`, `property_schema {"type":"object","properties":{"repository":{"type":"string"}}}` (open). One migration file may carry both 9.1 and 9.2 statements.
