...
**Built (web, per-object "governed here" line) (2026-08-23):** the other deferred half of Q4-A. `registry-detail.tsx` — the one generic object-detail page every registry type (`domains`/`services`/`assemblies`/`components`/`deployment-targets`/`teams`/`groups`/`users`/`service-accounts`) renders through — now carries a read-only line, `GovernedHereLineForObject` wired to `client.governanceMove.enforcement(registry.typeId, object.id)`, keyed off the page's own `detailKey` so the read fires exactly once per object shown. Rendered ONLY on a successful `enforced: true` answer: "Moves here are governed — enforcement enabled at ⟨tier⟩ '⟨name⟩'" naming the NEAREST rung on the object's chain (rungs arrive org-root-first per the schema doc, so the last entry is nearest; a second-or-later rung is "+N more" in the tooltip, never silently dropped), or "the instance level" when the instance rung alone is doing the work and no container rung exists to name. Pending, an errored read, and a successful `enforced: false` all render NOTHING — the line makes a claim only when it has one, and its absence is never itself a claim. Links to `/admin/governance`. Pure rendering (`GovernedHereLine`) split from the query wiring (`GovernedHereLineForObject`) so both are testable provider-free, `registry-detail.test.tsx`'s house pattern for pages that need a live router to mount directly.

Same round, same file: a **Delete…** action for every registry type (owner decision 2026-08-18), mirroring the confirm-dialog shape `admin-governance.tsx`/`admin-dependencies.tsx` already use for their own dialogs. `DeleteObjectCard` (danger-styled trigger, last card on the page) opens `DeleteObjectDialogBody`, which names the type/name/URN and gates Delete on the object's OWN NAME typed back exactly; the write goes through the SAME typed-registry client the page already reads with (`getRegistryClient(client, registry).delete(idOrUrn)` — `client.components.delete`/`client.teams.delete`/etc., never the generic `/objects/{type}` door, which refuses `component`'s delete outright). A refusal — the container-delete guard's 409 (ruling 2, §9.3) or a plain 403 — renders the server's `problem.detail` sentence verbatim (never the bare RFC 9457 `title`) and the dialog stays open with no navigation; success invalidates the registry's list query and navigates to `/$basePath`. Tests in `registry-detail.test.tsx` (14 cases: the confirm-name gate, both refusal shapes rendered verbatim with the dialog staying open, success's exactly-once `onDeleted`, no optimistic removal); three mutations applied/reverted (drop the `enforced` gate, drop the confirm-name gate, read `.message` instead of `.problem.detail`) each reddening exactly the case it exists to catch.

**Built (IaC) (2026-08-23):** the deferred half of Q4-A, closing charter principle 3 for the lattice. Manifest collection `governanceMoveRungs: [{ subjectIdOrUrn }]` in `packages/schemas/src/coordination-as-code.ts` (`ManifestGovernanceMoveRungSchema`) with diff entry `PlanGovernanceMoveRungDiffEntrySchema` (`create`/`delete`/`noop` — **no `update`**: a rung has no value beyond existing, and the TIER is derived server-side from the subject's object type, never declared, because a manifest that could name a tier could name one the subject is not); `openapi.v1.json` + generated SDK regenerated and committed, python-diffed against `f0a819cc` at **80 ADDED, 0 REMOVED, 0 CHANGED**. Authoring surface `Stack.addGovernanceMoveRung(subject)` in `packages/coordination-as-code/src/construct.ts` — stack-level only, because `Service`/`Domain` come from the uniform `defineResourceConstruct` factory so a fluent `subject.governsMoves()` would have to sit on `ResourceConstruct` and would then be offered on `Component`/`Team`/`Policy`/`DeploymentTarget`, every one of which `POST /plans` refuses (the reasoning that puts `producesDependency` on `Component` alone). Server: `plan-diff.ts` computes the verdicts and `invalidGovernanceMoveRungDeclarations` refuses the two declarations a stack may not make (a subject it does not own — `governance_move_rungs` carries no labels, so ownership is inherited from the subject container — and a subject type that cannot carry a rung); `plans-repo.ts` resolves subjects at plan-compute, builds the prune pool through `listGovernanceMoveRungs` (the same read the Admin page uses), re-proves the guard at apply against the STORED diff, authorizes `policy:write` at-or-above **the subject** per entry with the real applying principal, and applies through **`governance/move-rung-write.ts`** — a new module holding the whole act (row + Decision + audit event) plus the `governanceMoveRungScopeCheck` pair, extracted from `routes/governance-move.ts` so the HTTP door and the IaC door cannot perform a fraction of the verb or drift on authority (`dependencies/producer-declaration.ts`'s shape, applied unchanged). **`governanceMoveRungs` is the SECOND collection where an ABSENT key means UNMANAGED** (`producers` is the first) and the reason is sharper: pruning a rung disables a governance bar and the symptom is an *absence of refusals*, so a forgotten key must never un-govern a subtree — the accepted cost being that `@scp/coordination-as-code` cannot disable a stack's LAST rung (`Stack.synth()` omits an empty collection; use the DELETE verb or hand-author `"governanceMoveRungs": []`). A disable under an enabled upper rung fails the apply with the verb's own 409. Tests: `apps/server/src/coordination-as-code/iac-governance-move-rungs.integration.test.ts` (10 cases through the real plan+apply HTTP doors, incl. an Operator refused `policy:write` on the enable AND on the disable, the monotone 409, and an Operator then refused a `setService` move under the IaC-written rung while an Administrator makes the identical move — the case that proves the row means something) plus five construct cases in `packages/coordination-as-code/src/construct.test.ts`; six mutations applied/reverted with their measured messages recorded in each file's header.

**Reviewed and extended (2026-08-23):** two review findings, both fixed. (i) The apply-time authority covers every non-noop entry, but only the CREATE direction was pinned — narrowing `prepareApplyChecks`'s rung loop to `entry.action === "create"` left all 44 tests of the three plausible suites green while an org-root Operator holding `policy:write` nowhere could DISABLE a standing rung, the worse direction because a bar that is off announces itself only by an absence of refusals. Case (6)(c) now pins it, and the mutation reddens it alone. (ii) `scp plan`'s human table was built from four of the diff's seven collections while `summary` counted all seven, so a plan whose only content was a rung `delete` printed an EMPTY table under `deletes=1` — the row saying *this disables the governance:move bar on service X* being exactly the missing one. Fixed by property rather than by instance: `planDiffEntries` (exported from `packages/cli/src/cli.ts`) flattens every collection, `diffEntryRow` gained explicit `placement` / `dependency-producer` / `governance-move-rung` branches (the old fall-through mislabelled any unknown kind as a `relationship` with undefined refs), and the `PlanDiffEntry` union now ends in a `never` binding so adding an eighth collection without a table branch is a COMPILE ERROR. Pinned in `packages/cli/src/plan-diff-row.test.ts` by a count over the whole diff, not per kind.
