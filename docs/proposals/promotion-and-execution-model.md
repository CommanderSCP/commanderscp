...
| **scan + SBOM** | **coordinated Trivy step** (in Argo Workflows) | One pass emits both the vulnerability scan and the SBOM. Results made available to the commander as **gate evidence** — not a registry feature (see §4, ADR-0013). *(**Evolved 2026-07-23**, owner decision — M13: managed scanning is a first-class commander service; the promotion scan step executes **at the commander**; org-pipeline scan evidence remains a supported ingress. See `airgap-cds-validate-promote.md` §13.3.)* Pass-criteria is **scoped** (platform → trust domain (partition) → org → containment domain → service → component, most-restrictive-wins — ADR-0016). |
...
3. **The commander consumes the scan verdict** for the cross-boundary artifact(s) it gates (the coordinated Trivy step runs at the origin; the commander does not run it — **evolved 2026-07-23**: the first-class promotion scan step now executes at the commander, M13 §13.3) — see §4. *Domain-specific outpost artifacts are never scanned (they don't cross a boundary).*
4. **SBOM is emitted by the same coordinated Trivy pass** at build time — an **executor** output the commander consumes and references (SCP never runs the pass itself, §0 — **evolved 2026-07-23**: the charter-enumerated `scp-managed-scan` promotion scan step runs it at the commander when org pipelines do not, M13 §13.3).
...
- **Exception:** domain-locally-originated artifacts (outpost-owned, §2) never cross a boundary, so they have **no transfer phase and nothing to validate** — a shorter pipeline. The "always-shown boundary segment" and "universal validation" apply to cross-boundary changes only.
