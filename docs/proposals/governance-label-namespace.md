...
**Status:** **Accepted — superseded as the decision record by [ADR-0034](../adr/0034-governance-label-namespace.md)** (owner sign-off 2026-09-20). This document remains the CONTEXT doc it always was: the full census (§8), the measured re-parenting reproduction (§7a) and the withdrawn §5e live here and are referenced from the ADR rather than copied into it. The code implements §5a–§5d; **§5e is WITHDRAWN** (superseded by PR #244 — read it, it records a measured mutation that failed to kill its case). §7 lists what this deliberately does not do and §8 the census instances left open.
