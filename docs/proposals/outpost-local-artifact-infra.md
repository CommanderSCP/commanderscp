...
- Images are **scanned + signed at the source** (commander-side Harbor + cosign, already produced at M4/M6/M8). The **digest + signature + scan attestation** ride the *metadata* federation/promotion bundle; the outpost's **local Harbor holds the image bytes** and **verifies them against the signed digest+attestation before deploy — no re-scan**. The Trivy scan-gate stays commander-side (M11.4).
