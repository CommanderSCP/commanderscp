...
> **Scan location evolved 2026-07-23** (owner decision, M13): the one scan now executes **at the commander** as the promotion scan step; receivers (outposts, retrans) still **never re-scan** — they validate the commander's signature, the transitive proof of scan-pass. See `airgap-cds-validate-promote.md` §13.3. The receiver-side semantics in this section are unchanged.
...
- **The artifact-bytes transport was the honest open gap — now closed by [ADR-0019](../adr/0019-artifact-byte-channel.md)** (per-tier byte channel: commercial deploy-tool pull, operator-loaded MVP done via M17.4b, retrans validate-then-relay to build as M15.5(c)). Because bundles are metadata-only, the image/manifest *bytes* must still reach the outpost's local Harbor/Gitea by some channel. The two options originally weighed (both now scheduled — (a) shipped, (b) designed):
...
- **P2 — Harbor as a real observe target**: the missing auto-wire token hook + a registry executor reading image/scan state; scan-attestation verification (trust-scan-at-source — location evolved 2026-07-23, signature-validation semantics unchanged) as gate evidence.
...
- **P5** — artifact-bytes transport across the boundary (retrans/CDS relay). *(Originally "optional/later … if operator-loaded bytes prove insufficient"; the owner's finish-all-of-M15 mandate, 2026-07-20, makes it required — designed as the M15.5(c) validate-then-relay in [ADR-0019](../adr/0019-artifact-byte-channel.md).)*
...
2. **Artifact-bytes transport → operator-loaded + SCP-verify, first.** No real fork: bytes arrive via the existing air-gap install-bundle path and SCP verifies each promoted digest against the signed source attestation; the retrans/CDS byte-relay is a separate track — since made **required** and designed as M15.5(c) ([ADR-0019](../adr/0019-artifact-byte-channel.md)); operator-loaded remains the MVP row and sneakernet fallback.
