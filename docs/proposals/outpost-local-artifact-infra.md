...
- Images are **scanned + signed at the source** (commander-side scan + **cosign**). *(Superseded/corrected: the registry is **Gitea**, not Harbor — ADR-0012; cross-boundary **cosign** signing is **new** work — [ADR-0015](../adr/0015-cosign-cross-boundary-signing.md) — not a pre-existing M4/M6/M8 capability, which was Ed25519.)* The **digest + signature + scan attestation** ride the *metadata* federation/promotion bundle; the outpost's **local registry holds the image bytes** and **cosign-verifies them against the signed digest+attestation before deploy — no re-scan**. The Trivy scan-gate stays commander-side.
