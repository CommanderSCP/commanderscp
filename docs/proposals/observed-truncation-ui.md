...
5. **Whole-state elision — all three rungs** (`{__scpElided: string | true}`, or `observed: null` with the cut signalled) → version slot shows `Badge unknown` **"observed state truncated"**, the diagnostic sentence in the tooltip when rung 1 provides one; rollout slot stays empty (covered by the same pill). `observed: null` WITHOUT a cut signal = never observed = today's placeholder.
