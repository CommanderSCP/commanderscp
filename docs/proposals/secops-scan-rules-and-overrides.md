...
- **(c) There is no SecOps identity.** No security-specific permission exists; above-org floors use a shared `x-scp-operator-token`, org-and-below needs `policy:write` (Administrator/Owner only). There is also **no way to grant a role at all** — `role_binding:write` is declared and nothing checks it.
  > **Both halves closed 2026-08-27, kept above as the statement of the defect.** `drizzle/0099` splits out a `scan:override` permission and seeds a `SecurityOfficer` role that holds it alongside `policy:write` and **no** `object:write` (role-model.md §3A, ruling D3) — the SecOps identity this bullet says does not exist. `routes/role-bindings.ts` (§5 step 5) is the door that checks `role_binding:write`, so that role can actually be granted. The instance tier is **not** closed: above-org floors still ride the shared `x-scp-operator-token`, which is role-model.md §1.5 and its own milestone.
...
| D5 | **Administrator serves as SecOps for now**; a least-privilege security role comes later. **— DISCHARGED 2026-08-27.** The "later" arrived ten days after this table was written: the purpose-shaped role model ([docs/proposals/role-model.md](role-model.md), PR #307, migration `0099`) seeds **`SecurityOfficer`** — `policy:write` + `scan:override` + `freeze:write` over read-only everything else, holding **no `object:write`** (the whole point: the cumulative ladder could not express scan-override authority without also handing over the estate) and no `secret:write` (credential custody is an operations act, not a compliance one). `bindable_at` is `organization` only, and mechanically so — `OVERRIDE_APPROVAL_TIER_FLOOR = 'org'`, so a binding below the org root would mint waivers that are approved, audited and **inert**. `scan:override` is demanded at the decide door (`routes/scan-override-grants.ts:218`) **in addition to** `policy:write`, never in substitution, which is what makes D3's "approver = the tier that set the rule" and this role's least-privilege split hold at once. Ruling D3's "no sixth `ScanWaiverApprover` role" is satisfied by the same role holding both. |
