...
- **A4. Freezes**: freeze management is CLI-only at both roles, and `outpost-configuration.tsx` explains where freezes live while offering zero controls. A freeze card (declare/list/lift, scope picker) is the workbench's governance surface.
  - **CORRECTED 2026-08-24 (M25.7, owner decision D6).** This bullet used to open *"an outpost's freezes are **structurally** domain-only — no journal kind exists for them at all (measured; `service-board-precedence.integration.test.ts`)"*. That was measured and it was true: a freeze was a projection row with no graph object, so nothing could carry it. D6 retracted the conclusion ([ADR-0043](../adr/0043-federated-org-tier-freezes.md)) — an org-tier freeze declared `federate: true` gets a `freeze` graph object and rides the existing `object_upsert`. The *"no journal kind"* half is still true and is exactly why the design took this shape.
  - **What this changes for the card.** It must render whether a listed freeze is locally declared or a replica (read `Freeze.objectId`, then that object's origin — never inferred from role), because a replica **cannot be lifted here**: `DELETE`/`PATCH /freezes/{id}` answer **409** and the remedy is `freeze:override` at that freeze's own scope. An outpost declaring its own freeze may pass `domainLocal: true` so it never travels upward.
