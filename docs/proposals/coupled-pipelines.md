...
- **The release edge also runs the M17.4b pre-deploy artifact gate.** `waiting -> executing` is a second edge into `executing` beyond the ordinary `coordinated -> executing` path, so `advanceWaitingChanges` runs `runPreDeployArtifactGate` before evaluating requirements, exactly as `advanceExecutingChanges` does — defense in depth for the invariant "no change carrying a verified cross-boundary manifest enters `executing` without per-artifact byte verification" (M17.4b, closed in #108), independent of the fact that a promoted change never actually reaches `waiting` today (`requires` is stripped on import, §8 Q2).
