...
**Status:** Accepted — implemented (2026-07-22). Phases 1–4 (§7) all shipped: the CI-emit contract, the two security closes, the coupling engine (`waiting` state, the sweep, the two Decisions, the SLA), and the operator ergonomics (`wait-status`, "did you mean?", the key-reuse warn, this pipeline-author guide). See §10 "As built" for where the shipped implementation deliberately diverges from this doc's design and why. P4C (cascading rollback, §5) remains named future work, not built.
...
| `invalidatePropertyValidatorCache` guards a `property_schema` edit | **Was false — it had zero callers** (repo-wide grep returned only the definition). The compiled-validator cache was keyed on `type.id` and never invalidated, so a `property_schema` edit did not take effect in a running process. This proposal's tightening (§3.6) landed via **migration + process restart**, so it never bit us here. **FIXED since:** the cache is now keyed on the schema's content hash, so a changed schema is a different key and staleness is unrepresentable; the invalidator is deleted rather than wired up, because the only thing that rewrites `property_schema` is a migration and on the Helm split topology those arrive from `migrate-bin.ts` — a separate short-lived process that cannot reach the serving pods' heaps. See `graph/property-validation.ts` and `graph/property-schema-live-edit.integration.test.ts`. |
...
>
> **§10 supersedes this paragraph.** The shipped `requiresOf` does not throw — it collects a malformed entry and the routing guard PARKS the change in `waiting` (24h SLA), not `coordinated` (5-minute SLA). This paragraph is left as originally written, as the as-designed record; do not read it as current behaviour.
...
> **§10 supersedes point 1.** The shipped `coordinated -> waiting` transition does not pin this per-requirement structure — it carries a `reason` string plus an optional `malformedRequires` array. `everProvidedAtScope` is not pinned at entry at all (it is computed live, at read time, by `listProvidedKeysAtScope`'s "did you mean" — see §3.7/§10); nothing pins a cancelled ex-provider's state. This paragraph is left as originally written, as the as-designed record.
...
Ajv is `{allErrors:true, strict:false}` (`property-validation.ts`) — `strict:false` disables meta-checks only; `required` and `additionalProperties:false` **are** enforced, at `objects-repo.ts` (create) and (update), which covers `POST /changes`, the webhook processor, **and federation import**. At the time of writing this worked only because the schema arrived by migration + restart; a schema edit reaching an already-running process did **not** take effect (§1.7). That is no longer a caveat — the validator cache is content-addressed, so an edit applied by a migration to a live deployment is honoured by the next write with no restart.
...
| 4 | **Typo'd key** (`feture-a@us-east-1`). | Waits forever, mechanically indistinguishable from a slow prerequisite. `everProvidedAtScope: false` is set at entry; `wait-status` lists the keys actually provided at that scope; the watchdog warns once at 24h. | **Our weakest point.** We have replaced a fast silent *wrong* deploy with a slow silent *absent* one, and the push signal is weak (see #7). The `at` half is protected — a typo there is a **400 at emit** — so the surface is halved, not closed. **§10: `everProvidedAtScope` is not pinned at entry as designed here — see §10's annotation of §3.6.** The "keys actually provided at that scope" list (as shipped, `listProvidedKeysAtScope`'s "did you mean") stands as described. |
...
| 14 | **Junk `requires` from a version-skewed peer.** | Rejected at write by the tightened `property_schema` on the federation import path (`objects-repo.ts:142`). If it somehow lands, `requiresOf` throws → the change stays `coordinated` → flagged at **5 minutes**. Both layers fail closed. | Fixed the panel winner's SQL/TS inconsistency (its SQL said release, its narrower said throw). **§10 supersedes the "if it somehow lands" outcome:** the shipped `requiresOf` does not throw — it returns the entry under `malformed`, and the change PARKS in `waiting` (24h SLA, named in `wait-status`), not `coordinated` (5-minute SLA). Still fail-closed; see §10 for why parking beats throwing here. |
...
- **Stage** (beta/gamma/prod) has **no home in the object model yet.** The `domain` object type — the one middle scope the containment walk already honors (`domain_id` is route 1) — is currently an **unpopulated slot**: nothing outside tests creates a `domain` object (`objects-repo.ts:129` defaults every object's `domain_id` to the org root). It is a reasonable home for a stage *scoping node*, but the "freeze all of prod for free" framing an earlier draft of this section claimed is **wrong**, and the correction is instructive (verified against code, not the earlier draft):
  - **Nesting is via `domain_id`, not `contains`.** `contains` is registered `from_types=['service'], to_types=['component']` (`0021_service_contains_component.sql:24-27`), so `prod --contains--> prod-us-east-1` is **rejected at write** (`relationships-repo.ts:116-125`) and there is no config API to widen a builtin relationship type — only a migration. The mechanism that *does* work is setting the region-DT's `domain_id` to the stage-domain's id: `resolveDomainId` accepts any in-org object with no type check (`objects-repo.ts:131-135`), and `objects.domain_id` has no FK/CHECK. So `region-DT → stage-domain → org-root` resolves for policy, freeze, and RBAC — a freeze scoped at `prod` **does** reach the region deployment-targets under it.
  - **But it does NOT reach the components deployed there.** A component links to its region only via `hosted_on` (`component --hosted_on--> region-DT`), and **no scope walk follows `hosted_on`** (its sole consumer is `named-queries.ts:40`, a read-only impact traversal; `deploys_to` has zero consumers). A component's own `domain_id` defaults to the org root. Components and services are stage-**agnostic** definitions — one `payments-api` deploys to beta *and* gamma *and* prod — so they are not, and should not be, parented under any single stage. **What is "in prod" is a *deployment* — a change targeting a component into a region — not the component itself.** Neither a deployment-target nor a change carries a scoped `(stage, region)` today, so "freeze all of prod *releases*" is not a containment-scope operation at all: it needs the change to know its stage and region, which is the same gap called out below. The empty `domain` slot gives stage a scoping node; it does **not** by itself make "stop everything in prod" work.
  - The alternative — stage as a deployment-target — is strictly worse: a `deployment-target` gets **no** governance reach in any direction (nothing walks toward it; `hosted_on`/`deploys_to` are unwalked), so a freeze scoped there reaches nothing.
...
Phases 1–4 (§7) all shipped, verified by `coupling.integration.test.ts`, `transitions.integration.test.ts`, `change-authority.integration.test.ts`, the federation strip round-trip, and `transitions.test.ts`. Six places the shipped code deliberately diverges from this document's design, recorded so a future reader trusts the code over the prose above:

- **Decision `kind` is `"transition"` with structured `inputContext`, not `kind:"coupling"` (§3.6).** The `waiting -> executing` release Decision is written by the same `transitionChange` every other edge uses (`reconcile.ts`'s `advanceWaitingChanges`), carrying `extraInputContext: { satisfiedRequirements, ambiguousProviders? }` rather than a bespoke `kind`. One write path for every state change (not two, one generic and one coupling-specific) was judged simpler per principle 7, and `kind`/`verdict` being free-form (§1.7) meant nothing downstream needed the distinct label — `scp change explain` reads `fromState`/`toState` off `inputContext`, not `kind`. **The `coordinated -> waiting` entry Decision's payload is also reduced from §3.6 point 1's design:** it carries a plain `reason` string (e.g. `"waiting on 2 unsatisfied prerequisite(s): feature-a@<uuid>, bucket@<uuid>"`) plus, only when applicable, `extraInputContext: { malformedRequires }` — not the per-requirement `{key, at, atName, provider, providerState, everProvidedAtScope}` structure §3.6 specifies. Nothing in the shipped Decision pins a cancelled ex-provider's id or state, and `everProvidedAtScope` is never pinned at entry — the equivalent "did you mean" information is instead computed live, at read time, by `listProvidedKeysAtScope` off `explain`/`wait-status` (§3.7), not persisted into the Decision. Judged sufficient because Decisions are pinned historical records and live status is deliberately re-read at read time (§3.6's own stated principle) — the reason string plus `wait-status`'s live re-check together cover the same operator need with one less write path to keep in sync.
- **The sweep is a guarded forward poll with round-robin fairness, not §3.5's inverted double-`NOT EXISTS` query.** The starvation hazard §3.5 flags in its own uncertainty note turned out real enough to design around directly rather than gamble on the planner: `advanceWaitingChanges` still asks "which waiters are releasable" per-row (`requirementStatuses` under the same `tryAcquireChangeCoordinationLock` pattern as every other sweep), but a still-unsatisfied waiter gets its `updated_at` bumped every tick, so `ORDER BY updated_at ASC LIMIT` can never let one stuck waiter monopolize the batch head — the same failure mode §3.5 names, closed without depending on `obj_props` being chosen for a correlated-subquery `@>` inside a single inverted SQL statement.
- **`requires` lives on the typed request schema with propose-time stripping of smuggled properties, not §3.6's `property_schema` tightening.** `object_types.property_schema` for `change` was left permissive; instead `proposeChange` (`changes-repo.ts`) resolves every `{key, at}`'s `at` to a real object id at propose time (404 on typo, per §3.7's own goal) and explicitly strips any caller-supplied `provides`/`requires` that arrived via the free-form `properties` bag rather than the typed field, so the only values ever stored are the resolved ones. This gets the same fail-closed guarantee (an unresolved `at` can never reach storage) without a schema migration, and keeps `requires` validation next to the one place that already needs to resolve `at` objects (the target-resolution loop) instead of a second enforcement point in Ajv. `provides` keeps a `properties` fallback (federation replay still needs it); `requires` does not, because federation promotion strips it outright (§8 Q2) and no legitimate caller writes it any other way.
- **The release edge also runs the M17.4b pre-deploy artifact gate.** `waiting -> executing` is a second edge into `executing` beyond the ordinary `coordinated -> executing` path, so `advanceWaitingChanges` runs `runPreDeployArtifactGate` before evaluating requirements, exactly as `advanceCoordinatedChanges` does on the ordinary edge — defense in depth for the invariant "no change carrying a verified cross-boundary manifest enters `executing` without per-artifact byte verification" (M17.4b, closed in #108), independent of the fact that a promoted change never actually reaches `waiting` today (`requires` is stripped on import, §8 Q2).
- **The `0007` `state_transitions` seed drift is fixed, but the table stays checked-but-unread.** Migration `0032_state_transitions_waiting.sql` inserts the three `waiting` edges as additive rows (0007 itself is never touched — its hash is verified on live deployments). Nothing at runtime reads `state_transitions`; `LEGAL_TRANSITIONS` in `transitions.ts` is the sole runtime authority, exactly as designed. What changed is that `transitions.integration.test.ts` — which 0007's own header had claimed existed since that migration was written — now actually exists and asserts set equality between the two, so the mirror table can no longer silently drift out from under a comment that says it's checked.
- **A malformed `requires` entry PARKS the change in `waiting` under the 24h SLA — `requiresOf` does not throw, contradicting §3.5's and §6#14's original text.** Both passages (annotated in place above) describe `requiresOf` throwing, the routing guard leaving the change in `coordinated`, and `coordinated`'s 5-minute SLA catching it. That is not what shipped. `requiresOf` (`changes-repo.ts`) is a narrow-and-collect function: a stored entry that fails to parse as `{key, at}` is appended to a `malformed` array rather than raising, and every reader — the routing guard in `advanceCoordinatedChanges`, the waiting-sweep predicate in `advanceWaitingChanges`, `wait-status`/`explain`'s `buildWaitStatus`, and the watchdog — treats a change carrying any `malformed` entry as UNSATISFIABLE: it (or stays) parked in `waiting`, where the 24h SLA flags it and `wait-status` names the bad entry verbatim. This is deliberately better than the throw §3.5/§6#14 describe, for two reasons. First, blast radius: `requiresOf` runs inside `advanceCoordinatedChanges`'s per-row loop over a whole batch of changes; a hard throw there is caught per-row (`logChangeError`, not a bare crash) but still aborts *that row's* transition for the tick, leaving it retried every tick with no more information than "it keeps failing" — collecting-and-parking instead lets the SAME tick record a `reason` naming exactly which entry is malformed, no retry-and-hope needed. Second, and more important: parking is fail-closed AND visible AND actionable in one state — the change never executes (fail-closed, same as the throw), the bad entry is named in `wait-status`/`explain` rather than requiring an operator to correlate a thrown-error log line back to a change id, and the 24h `waiting` SLA is the SAME SLA every OTHER unsatisfiable-prerequisite case already gets (an unmet `{key,at}` and a malformed entry are both "this change cannot proceed as declared," and treating them identically means one code path, one SLA, one diagnostic surface, instead of two). The 5-minute `coordinated` SLA the original text describes exists for a DIFFERENT failure class (plan-compilation failures that need fast attention because nothing is holding the change back) — reusing it here would have signaled "just retry," which is the wrong signal for a condition that only a human editing the stored `requires` can resolve.

P4C (cascading rollback, §5) remains named future work, not built.
