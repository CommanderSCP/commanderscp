...
- **4 — Surfaces. SHIPPED.** All of it, plus one more the review found. `explain` carries `stageDependencyStatus`; `scp change explain`/`scp change wait-status` print a stage-dependency section; the component-pipeline view paints a held stage `held` and names the dependency, in the card, on its own subnode and on the promotion arrow into the wave; the watchdog's `executing` stall notice names the coupling instead of blaming an executor that was never handed the target; and `GET /decisions` gained the `kind` filter this document's own federation strip had been promising in a comment (`scp decision list --kind stage_dependency`), with the index that makes it a probe rather than a scan of a 12M-row table.
  **The one rule every surface obeys: re-run the predicate, never read the Decision.** Nothing writes a clearing row, so the newest `stage_dependency` row of a change that was held and then released still says `hold` — for ever — and on an outpost the newest row of that kind is the promotion-import `allow` instead. A surface sourced from either would rebuild, on a read path, exactly the permanent-marker bug §3 chose `verdict: "hold"` over `"block"` to avoid. `resolveStageDependencyStatus` is the single read side all five share; it persists nothing, so the surfaces cost zero writes (ADR-0024).
