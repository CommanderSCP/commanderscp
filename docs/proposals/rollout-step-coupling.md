...
**Status:** v0.2 — **implemented and merged** (PR #220, 2026-08-05); [ADR-0028](../adr/0028-stage-scoped-component-coupling.md) is Accepted. **All increments 0–5 have now shipped**; increment 4 (operator surfaces) landed last, one surface per §5. v0.1 (2026-08-04) was written before the mechanism was grounded against code; **§0 records what that grounding refuted**, and the design below is a rewrite, not an edit. Owner rulings of 2026-08-05 are in §6.
...
- **4 — Surfaces. SHIPPED.** All of it, plus one more the review found. `explain` carries `stageDependencyStatus`; `scp change explain`/`scp change wait-status` print a stage-dependency section; the component-pipeline view paints a held stage `held` and names the dependency, in the card, on its own subnode and on the promotion arrow into the wave; the CHANGE-pipeline view marks a held wave target and names the dependency beside the raw `pending` its status column still reads; the watchdog's `executing` stall notice names the coupling instead of blaming an executor that was never handed the target; and `GET /decisions` gained the `kind` filter this document's own federation strip had been promising in a comment (`scp decision list --kind stage_dependency`), with the index that makes it a probe rather than a scan of a 12M-row table.
  **The one rule every surface obeys: re-run the predicate, never read the Decision.** Nothing writes a clearing row, so the newest `stage_dependency` row of a change that was held and then released still says `hold` — for ever — and on an outpost the newest row of that kind is the promotion-import `allow` instead. A surface sourced from either would rebuild, on a read path, exactly the permanent-marker bug §3 chose `verdict: "hold"` over `"block"` to avoid. `resolveStageDependencyStatus` is the single read side all of them share; it persists nothing, so the surfaces cost zero writes (ADR-0024).
  **And re-running a live predicate has a permanent marker of its OWN, which the first cut of this increment shipped.** The predicate does not know that the change is dead: a cancelled release's active wave is the dead one, and its never-run `pending` targets each still evaluate to a hold. The gate that separates a wait from an abandonment is the change's own state (`executing`, the only state in which reconcile evaluates the coupling at all) and it now lives INSIDE the resolver — it was written at one of the three call sites, so `explain`, `scp change explain` and `scp change wait-status` reported a cancelled change as held for ever while the component-pipeline view was correct. One question, one predicate, one place.
  **Still not done, and named rather than smuggled in:** a held change trips the 30-minute `executing` stall SLA and gets a `severity: "warning"` notification saying "stalled" about a feature working as designed. The notice now NAMES the coupling — the floor — but the SLA was not lengthened: the `waiting` arm's 24 h was given for exactly this shape, and keying one off "is held" needs a persisted state that is precisely the per-tick write ADR-0024 forbids. Wants an owner ruling. There is also no service-board signal for a hold: `ServiceBoardAttentionSchema`'s four fields are all required and both reusable ones would lie (`blocked` comes from the verdict-only query this ADR forbids reusing; `awaitingApproval` claims a human gate).
  **One measured inefficiency, left deliberately:** both read surfaces evaluate every untriggered target of the change's active wave, not only the target the caller asked about, and the evaluations are sequential. Parallelising them buys nothing — all three callers run on ONE tenant transaction, i.e. one connection, which is why `explain` awaits the plan before its `Promise.all` rather than inside it. Narrowing them to the caller's own targets would help the component-pipeline page, but it would make `held`/`unenforced` mean "among the targets you asked about", so a change genuinely held elsewhere would answer `held: false` to a narrow caller — a surface that lies by omission, which is the class this increment's review was about. The amplification factor is 1 for 277 of 281 measured changes (single-target, ADR-0026), and the honest fix is a per-target evaluator with its own name and its own contract, not a filter argument on this one.
