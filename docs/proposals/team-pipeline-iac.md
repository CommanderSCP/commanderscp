...
**Status:** Draft — proposed 2026-08-26. The rulings in §0 (D1–D15) were made by the owner in-session on 2026-08-26; the design built on them is pending owner review as a whole.
...
- **CLI/SDK accuracy sweep:** the recon found pipeline authoring has no dedicated CLI/UI surface at all; this proposal adds `scp config-source register|status` (single repo or `--repo-pattern` namespace, D9), `scp iac scaffold|export`, and (sugar) `scp pipeline …` over the generic routes, with the parity table verified per increment.
...
9. Sub-target modeling (the "select, don't create" menu): child deployment-targets under a stage (recommended — freezes, binding policies, and policies can then scope to a single cluster/instance group via the existing containment routes) vs. placement-property selection only. Related: artifact-class enum growth posture — additions land request-side first; check the oasdiff response-enum rule before echoing the class in responses. Note the menu targets are domain-**authored** but must federate (team placements at the commander resolve against them), so they live in a federating outpost-origin stack, not the domain-local bindings stack.
