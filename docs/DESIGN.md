...
| Security (enterprise-grade) | OIDC + PKCE; argon2; hashed tokens; RLS defense-in-depth tenancy; deny-override RBAC; credential asymmetry to executors (scoped exceptions only via the charter-enumerated managed classes: the isolated managed-IaC runner, and since 2026-07-23 the read-only managed-scan runner — §12, [ADR-0020](adr/0020-first-class-commander-scanning.md)); signed webhooks; cosign-signed releases; no runtime plugin loading. |
