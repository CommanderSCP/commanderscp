...
- **Freezes do not federate.** A freeze is a projection row rather than a graph object, so it cannot ride `object_upsert`, and `SyncJournalEntrySchema` has no freeze-shaped kind: a freeze declared at the commander is **not** a freeze at the outpost, and this is pinned by a test. Owner decision 2026-08-23 is that **org-tier** freezes should federate (as graph objects, with the projection rebuilt on import); that work is M25.7 and is **not built**. The **platform tier will not federate under any decision** — the sync journal is org-scoped at every layer and a platform freeze has no `org_id`, so it stays per-instance operator config distributed by deployment tooling.
