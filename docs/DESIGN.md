...
- **A freeze can be retracted.** `DELETE` lifts one immediately whatever `endsAt` says, and `PATCH` moves `endsAt` in either direction; both require `freeze:write` at the freeze's own scope and a mandatory reason. A lift is **soft** — the row stays readable forever, because gate and admission Decisions cite `freeze.id` in their `inputContext` and that citation has to keep resolving (principle 6).
