...
`scp-bundle-<version>.tar.gz` contains: all images — scpd, **all three managed-execution runners** (`scp-runner-iac`, `scp-runner-scan`, `scp-runner-dep`), the evaluation postgres, and the bundled Standard-Stack backends (Argo CD + Valkey, Gitea, Argo Workflows/Events, Mode B) — as **OCI layout** (skopeo-copy-able into any private registry), the Helm chart, the compose file, bundled plugins, checksums, **cosign signatures**, offline docs, and an **install/upgrade script that retargets image references to the customer's registry** (grafted — removes the most error-prone manual step of offline installs). The same bundle format is the upgrade package, so disconnected upgradeability is exercised from v1.
