...
- **Resolution:** walk the containment path org → **containment domain** → service → component; policies inherit downward; **stricter wins** on conflict; local domains may add strictness but never weaken higher-level requirements unless explicitly permitted (federated governance per charter). "Domain" here is the intra-org **containment domain** object type, not a federation/trust domain.
