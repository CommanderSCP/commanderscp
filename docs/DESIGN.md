...
- **Resolution:** walk the containment path org → **containment domain** → service → component; policies inherit downward; **stricter wins** on conflict; local domains may add strictness but never weaken higher-level requirements unless explicitly permitted (federated governance per charter). "Domain" here is the intra-org **containment domain** object type, not a federation/trust domain. *(Vocabulary reference: [GLOSSARY.md](GLOSSARY.md) — "security domain" vs "containment domain", [ADR-0021](adr/0021-terminology.md).)*
  - **Two tiers above org, for scan-requirements only** ([ADR-0016](adr/0016-scoped-scan-requirement-policies.md)): scan pass-criteria resolve over a six-tier chain — **platform → trust domain (partition) → org → containment domain → service → component** — most-restrictive-wins (per-severity MIN, order-independent). The two above-org tiers come from a single **instance-scoped floor table** (no `org_id`, operator-write / tenant-read, §4.2 exception), *not* from the org-rooted `containmentChain`, which structurally cannot reach above org. **This widening applies to scan-requirements only** — it deliberately does **not** widen freeze scoping or any other policy effect (§10.3), which stay org-rooted.
    - **A second, loosening dimension ([ADR-0033](adr/0033-scan-exclusions-and-overrides.md), 2026-08-17).** Beside the ceiling, individual **findings** may be **excluded before counting** — because the component is on the latest of that dependency's major line, because no fix exists, because the component declared a qualifying fact, or because an owner's **override request** was approved and has not expired. The two dimensions have deliberately different algebras: the ceiling merges by per-severity **MIN**, an exclusion by **monotone AND** down the same chain (a clause has effect at tier T only if every tier above admits its class), and **admission is empty by default at every tier**, so with nothing authored behaviour is byte-identical. Both are commutative, so the order-independence the tie at `containment.ts:59-73` requires holds across both. Exclusions resolve **per target and are never unioned across targets** — a union is safe for a tightening and an inversion for a loosening. `severityCounts` keeps meaning **what the scanner found**; a separate `effectiveSeverityCounts` carries the post-exclusion number, so no CEL condition already authored against the former changes meaning.
