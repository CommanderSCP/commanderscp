...
| Federation connectivity | All network federation is outpost-initiated (outpost pulls config, pushes status); the commander never dials an outpost. Commander view of air-gapped outposts = bundle-transfer tracking (submitted → confirmed) + last-known state, never presented as live | §13 |
