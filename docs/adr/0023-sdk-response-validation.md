...
- **`GET /events/stream` is not in the spec at all.** The SSE feed is not a generated operation, has no generated schema, and never passes through `responseValidator`. `apps/web/src/lib/use-event-stream.ts` does `JSON.parse(event.data) as RelayedEvent` and then dereferences `payload.subject` with no runtime check — a bare cast of network bytes, exactly the class this ADR abolishes everywhere else. That file's own comment already calls itself the one sanctioned exception; this ADR does not close it, and closing it is tracked separately (the SSE API-parity work, which brings the stream into the spec so the generator covers it like everything else).
