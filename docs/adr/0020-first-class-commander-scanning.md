...
- **E6 fail-closed universality.** A missing scan refuses exactly like a failed one, policy bound or not. The promotion scan step adds a way to *satisfy* E6; it adds no way to loosen it.
  > **AMENDED 2026-08-17 — the second sentence is SUPERSEDED *if and when* [ADR-0033](0033-scan-exclusions-and-overrides.md) is accepted (it is currently `Proposed`).** That ADR deliberately adds a way to loosen: a separately-authorized per-finding **exclusion** dimension, admitted top-down by a monotone AND down the tier chain, applied *before* counting. It was an owner decision, not a regression, and ADR-0033 §9 records the supersession explicitly. Until that ADR is Accepted, this bullet still governs.
  >
