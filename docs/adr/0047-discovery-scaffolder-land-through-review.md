...
- **`backfill-source-mappings` survives until the estate migration completes** (proposal §9, increment 7) and is then **removed the same way** — same exception process, same reasoning. It exists to repair estates imported the old way; when the homelab estate has converted, its subject no longer exists.
  - **Done, 2026-08-29** (ledger entry "team-pipeline-IaC increment 7 tail"). One correction to the reasoning above, worth stating because it is what actually made the removal safe: the gate was never a particular estate converting. Removing `accept` **closed the population** — it was the only door that could create a component without a source mapping — so the set the route repaired became finite and non-growing the moment this ADR shipped, and a component still in it is repaired by adopting it into a stack and declaring the source in the manifest.
