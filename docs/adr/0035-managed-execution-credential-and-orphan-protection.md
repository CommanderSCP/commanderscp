...
- **The grace is now derived from what teardown costs on the adapter in use.** `RUNNER_TEARDOWN_STEPS` declares how many bounded calls each adapter's `finally` issues (Docker 1; Kubernetes 3 — DELETE the Job, DELETE the Secret, remove the workspace subtree), `runnerTeardownWorstCaseMs`/`runnerPostDeadlineMs`/`runnerRunBoundMs`/`runnerReapGraceMs` derive every bound from that count, and `MANAGED_TRIGGER_GRACE_MS` becomes `runnerPostDeadlineMs(kind) + MANAGED_OUTCOME_TAIL_MS` with `kind` read from the **same server-injected `runnerLauncher` field `resolveRunnerLauncher` switches on**. Docker 62s; Kubernetes 124s — the number 60s was short of by two thirds of a teardown.
