...
**The docs already solved this; the code did not.** [ADR-0016 §Terminology](0016-scoped-scan-requirement-policies.md) mandates the full forms and bans bare "domain" even as a stored value ("the floor table's tier literal is `trust_domain`, never bare `domain`"); DESIGN.md does the same for policy resolution. But in the code, counted on `origin/main` at **`da9e92c`** (2026-07-24), there are **365** non-test source lines mentioning `domainId` across `apps/` and `packages/`, in **75** files — about **51** in `apps/server/src/federation` meaning the **trust** sense (`federation_self.domainId`) and about **45** in `apps/server/src/graph` meaning the **containment** sense (`objects.domainId`), both declared in `apps/server/src/db/schema.ts` — with **zero type-level separation**. Both are plain `uuid`. Nothing stops one being passed where the other is expected.
...
| **branded `TrustDomainId` / `ContainmentDomainId`** (D4, code) | 365 non-test `domainId` lines across 75 files (`origin/main` @ `da9e92c`); 51 federation (trust), 45 graph (containment); both plain `uuid` | **Moderate.** Type-only — no runtime behaviour, no schema, no API. Mechanical but wide. | Tracked follow-on PR (i) |
...
- `apps/web/src/routes/change-pipeline.tsx:26, 206, 208, 237, 310, 314, 398, 406` (`data-testid="pipeline-stages"`), `:407, :420` — all ten matching lines in the file (13 occurrences).
