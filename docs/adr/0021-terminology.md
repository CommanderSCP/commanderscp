...
**The branded-types work is done; this rename is done.** Follow-on PR (i) below landed the brands (PR #140); the `scopeKey` rename landed on top of it. What remains open under this ADR is (ii) and (iii-b).
...
(i) branded domain-id types; (ii) the `promote` → `accept` rename — **breaking**; (iii-a) the cheap `stage` cleanup; (iii-b) the `stage` cleanup's `/v1` half — **breaking**. Two of the four are breaking `/v1` changes. **(i) and (iii-a) have landed**; (ii) and (iii-b) are not started.
