...
> **ENVIRONMENT-VALUE VOCABULARY SUPERSEDED 2026-08-26 (D6 / D21(e), [team-pipeline-iac.md](../proposals/team-pipeline-iac.md) §0, accepted by [ADR-0046](0046-what-how-split-config-sources-and-binding-policy.md)). The grammar this ADR decided is UNCHANGED; only the illustrative `env` values are.** Going forward the tiers are spelled **`staging`** (replacing `gamma`) and **`production`** (spelled out, never `prod`); [docs/GLOSSARY.md](../GLOSSARY.md), which this ADR delivers, is swept accordingly and is the current reference. **D6's stage grammar itself — `<domain>[-<location>]-<env>`, the optional location segment, fixed segment order, lowercase, and the hyphen-free-segment rule — stands exactly as decided here**, so `commercial-amer-staging` parses under the identical rule that `commercial-amer-gamma` did.
>
