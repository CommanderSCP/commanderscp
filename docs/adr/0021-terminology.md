...
**The docs already solved this; the code did not.** [ADR-0016 §Terminology](0016-scoped-scan-requirement-policies.md) mandates the full forms and bans bare "domain" even as a stored value ("the floor table's tier literal is `trust_domain`, never bare `domain`"); DESIGN.md does the same for policy resolution. But in the code, counted on `origin/main` at **`da9e92c`** (2026-07-24), there are **367** non-test source lines mentioning `domainId` across `apps/` and `packages/`, in **75** files — about **51** in `apps/server/src/federation` meaning the **trust** sense (`federation_self.domainId`) and about **45** in `apps/server/src/graph` meaning the **containment** sense (`objects.domainId`), both declared in `apps/server/src/db/schema.ts` — with **zero type-level separation**. Both are plain `uuid`. Nothing stops one being passed where the other is expected.

*(Re-run: `git grep -n domainId origin/main -- 'apps/**' 'packages/**' | grep -v -E '\.test\.|__tests__|\.spec\.'`. Update the commit stamp with the number.)*
...
Rejected because it has already been tried. ADR-0016 and DESIGN.md both mandate the full forms, and the code still has 367 undifferentiated `domainId` lines (`da9e92c`) carrying two different meanings. A convention that two documents already mandate and the code still violates is not a control. D4 makes it a compiler error instead.
...
| **branded `TrustDomainId` / `ContainmentDomainId`** (D4, code) | 367 non-test `domainId` lines across 75 files (`origin/main` @ `da9e92c`); 51 federation (trust), 45 graph (containment); both plain `uuid` | **Moderate.** Type-only — no runtime behaviour, no schema, no API. Mechanical but wide. | Tracked follow-on PR (i) |
...
An earlier draft of this ADR named only two misuse sites (`StageCard.tsx`, `change-pipeline.tsx`) and priced the whole cleanup as "cheap — UI and docs only, no API, no schema, no migration". **That census was incomplete and that cost claim was false.** Three subsequent review passes each found further misses in the corrected census, so the census is now stated with its **method** attached rather than merely asserted to be complete.

**Census method (re-runnable).** Case-insensitive `stage` across `apps/`, `packages/` and `tools/openapi`, excluding `*.test.*`, `*.spec.*` and `__tests__`, on `origin/main` at **`da9e92c`** (2026-07-24). Every hit is accounted for below — in (iii-a), in (iii-b), or in the explicitly-out-of-scope list. Where a file is the misuse rather than a line in it, it is named as a whole file and said to be so; where a list is not exhaustive it says "and others". [docs/GLOSSARY.md](../GLOSSARY.md)'s `stage` entry carries the identical census, and the two must be corrected together.

**(iii-a) The cheap half — UI labels, test hooks, comments and prose. No API, no schema, no migration. Not started.**

- `apps/web/src/components/pipeline/StageCard.tsx` — **the whole component**, 28 occurrences, not enumerated line-by-line because the file itself is the misuse: docblock `:90` (*"One pipeline stage = one compiled wave"*), `stageNumber` prop (`:99, :103`), the visible label `Stage {stageNumber}` (`:116`), `data-stage` (`:111`) and nine distinct `data-testid="stage-*"` hooks (ten occurrences).
- `apps/web/src/routes/change-pipeline.tsx:26, 206, 208, 237, 310, 314, 398, 406` (`data-testid="pipeline-stages"`), `:407, :420` — all ten occurrences in the file.
...
- "stage" for a pipeline **phase**: `apps/server/src/coordination/change-coordination-lock.ts:6`; `tools/openapi/check.sh:3, 10` and `tools/openapi/README.md:9`, which name "BUILD_AND_TEST.md §6 stage 3" — fixing those means renaming that section in BUILD_AND_TEST.md too, so they are not free the way a comment is; and prose such as the execution map in `docs/proposals/promotion-and-execution-model.md` §1. Say **phase** or **step**.
- "stage" for a **milestone sub-step** — two variants, counted separately because they need different greps:
  - the **`M<n> stage` form: 37 sites in 18 files** — `apps/server/drizzle/0004_auth_expansion.sql`, `apps/server/drizzle/0005_plans.sql`, `apps/server/src/app.ts`, `apps/server/src/auth/device-flow.ts`, `apps/server/src/auth/local-auth.ts`, `apps/server/src/auth/oidc.ts`, `apps/server/src/auth/pat.ts`, `apps/server/src/config.ts`, `apps/server/src/db/schema.ts`, `apps/server/src/routes/auth.ts`, `apps/server/src/routes/device-flow.ts`, `apps/server/src/routes/oidc.ts`, `apps/server/src/routes/pats.ts`, `apps/web/vite.config.ts`, `packages/schemas/src/auth.ts`, `packages/schemas/src/graph.ts`, `packages/sdk/src/client.ts`, `packages/sdk/src/index.ts` — the complete roster, not a sample;
  - **prefix-less variants, 3 sites** an `M<n> stage` grep misses: `apps/web/src/routes/device.tsx:12`, `apps/web/src/routes/pats.tsx:26`, `apps/web/vitest.config.ts:14`.
...
- `packages/schemas/src/services.ts` — `ServiceBoardStageSchema` (`:29`), the exported `ServiceBoardStage` type (`:37`), `ServiceBoardRowSchema`'s `currentStage` (`:72`) and `stages` (`:73`), and the row docblock defining `currentStage` as the wave's display name (`:62`). The docblock at `:25` is explicit: *"One pipeline stage of a component's latest change = one compiled wave"*.
- `apps/server/src/routes/services.ts` — shipped on `GET /api/v1/services/:idOrUrn/board`; the route's leading comment says "per-stage status" (`:25`) and so does its OpenAPI `summary` (`:36`).
- `packages/sdk/src/index.ts:68` (re-export); `packages/sdk/src/generated/types.gen.ts:6158–6159` (the generated field pair); `packages/sdk/src/generated/sdk.gen.ts:876` (the same route summary as the generated operation docstring). All three regenerate from the contract.
- `tools/openapi/openapi.v1.json:23130` (the route `summary`), `:23236` (`currentStage`), `:23246` (`stages`), and the latter two in the **required** list at `:23393–23394` — the document is committed, so the change is visible to the gate.
- `apps/server/src/coordination/service-board.ts:6, 110–111, 126, 138, 141, 157–158` — the server-side projection.
- `apps/web/src/routes/service-board.tsx:3` (`import type { ServiceBoardRow, ServiceBoardStage } from "@scp/sdk"` — the import the rename breaks), `:38–59` (`StageStrip`; its `board-stage-badge` at `:51` is captioned from the **wave** index — `Wave ${s.waveIndex}` at `:50`, `W${s.waveIndex}` at `:53`), `:190–191` (`<TableHead>Current stage</TableHead>`, `<TableHead>Stages</TableHead>`), `:248–249` (`row.currentStage`), `:255` (`row.stages`).
...
- Docker's own multi-stage-build term: `apps/runner-scan/Dockerfile` (**eight occurrences — the largest single concentration in the tree**, and a real two-stage build: `STAGE 1 — Trivy`, `STAGE 2 (FINAL) — OpenSCAP`); `apps/runner-scan/README.md:19`, which describes that build; `packages/cosign/src/cosign-bin.ts:5`; `packages/cosign/src/skopeo-bin.ts:5`; `packages/plugin-testkit/src/runner-image.ts:36`.
- The unrelated verb "staged": `apps/server/src/governance/scan-db.ts:399, 404, 409`.
- `apps/server/src/graph/named-queries.ts:279`, whose hypothetical "stage-domain" is consistent with the reserved place sense.
...
- **Citation discipline, adopted after three review passes found more stale line numbers and counts than wrong ideas.** In this ADR and in the glossary, a **line number** appears only where the exact line *is* the evidence — a verbatim quoted comment, a specific enum value, a schema field definition, or an entry in the `stage` census, which is a census *of lines*. Every other reference — "in the code" pointers, file rosters, see-alsos — cites the **file path alone** and names the symbol, because paths and symbols survive a rebase and line numbers do not. Every **count** states the commit it was measured at, inline, so a future reader can distinguish a stale number from a wrong one. Both documents follow this; keep it when editing either.
