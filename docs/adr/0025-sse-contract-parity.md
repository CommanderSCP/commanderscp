...
- **Oversized rows.** Postgres's `NOTIFY` payload is capped near 8000 bytes. A serialized `RelayedEvent` at or under 7000 bytes goes out whole; over that, the relay sends a small `{id, orgId, oversized: true}` marker instead, and the bridge fetches that one outbox row by id under the same narrowly-scoped `SET LOCAL ROLE scp_relay` escalation the relay itself uses (D-nothing new here security-wise: `scp_relay` is `NOBYPASSRLS`, granted only `SELECT`/`UPDATE` on `outbox` — extending that reviewed escalation to every SSE-serving process widens no blast radius, since the row was already readable via the same channel had it merely been smaller).
