...
**Status:** Proposed (2026-07-18) — **corrected 2026-07-19** (see note below)
...
3. **Signed promotion manifest**: the commander uses **cosign** — the **new** cross-boundary signing layer decided in [ADR-0015](0015-cosign-cross-boundary-signing.md), **not** the pre-existing Ed25519 federation-transport layer — to sign an enumeration of exactly the authorized artifact set (digests) — the artifact(s), the SBOM, and the manifest. Signing happens **only if scans pass**. (Ed25519 stays in place for the federation bundle envelope / journal / attestations; cosign is the additive supply-chain layer — see ADR-0015 §1.)
...
**Positive**
- One Trivy pass yields both scan verdict and SBOM. Signing reuses the **proven keyful/offline cosign wrapper** from the release path (`deploy/airgap/src/cosign.ts`) — cosign is already vendored — but applying it to cross-boundary artifacts + the manifest is **new work** ([ADR-0015](0015-cosign-cross-boundary-signing.md)), **not** a pre-existing M4/M6/M8 capability (that was Ed25519).
