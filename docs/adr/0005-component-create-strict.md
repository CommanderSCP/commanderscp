...
> **AMENDED 2026-08-26 by [ADR-0047](0047-discovery-scaffolder-land-through-review.md) — the *import-permissive* half only; create-strict is untouched.** The principle "create is strict, import is permissive" becomes **"create is strict; scaffold permissively, land through review."** `POST /discovery/accept` — the surface this ADR's Decision meant by "import surfaces stay permissive by design," and the platform's only observation-driven graph-write path — is **removed**, and discovery becomes a scaffolder that emits `@scp/coordination-as-code` construct code and a manifest for a human to review and merge. The spirit is preserved: nothing blocks a user from bringing an estate in, and the scaffolder applies no strictness to its *proposal*; what moves is the **graph write**, which now goes through the reviewed manifest path where create-strict applies like it does everywhere else. **Precisely two of the three surfaces this ADR called "import" are unaffected — federation journal replay (single-writer authority materializing another domain's authored state, not a user import) and overlay are unchanged.** Everything else in this document — the required `service` field, the generic-route refusal, the one-time `/v1` break and the exception process that carried it — still governs. Motivating measurement: the remedy this ADR paired with permissive import (organize-after's `move` verb) did not run in practice, and the homelab's ~50 imported components remain RBAC orphans; existing estates convert via adopt/export rather than by re-import.
...
**Accept the break.** `POST /components` (and the create branch of `PUT /components/{urn}`) require `service`. The generic `/objects/component` route refuses all write verbs (403); the strict route is the only direct way to create a component. ~~Import surfaces stay permissive by design.~~ — **superseded 2026-08-26 by [ADR-0047](0047-discovery-scaffolder-land-through-review.md): scaffold permissively, land through review.** `discovery/accept` is removed and its output becomes reviewable IaC; federation journal replay and overlay are unchanged. See the amendment note at the top of this document.
