...
### 8. The D7 rollback exemption stops at the tier boundary (added by adversarial review)

Owner decision D7 exempts a **rollback** from a freeze at the wave boundary, because holding one pins a broken release in place for the length of the window and `scp change rollback` is the documented exit. M25.3 shipped that exemption **tier-blind** at both of its seams (`gate-orchestrator.ts`'s `freezeExemptRollback`, `reconcile.ts`'s per-target `continue`), which made it a hole rather than a decision: `POST /v1/changes/{id}/rollback` authorizes on `object:write` **at the org** and nothing else — no `freeze:override`, no mandatory reason, no operator token — so a tier-blind D7 handed every principal who can write an object a route past the freeze whose own block sentence promises *"no tenant role can override it, however privileged"*, and a **cheaper** route than the override it was being contrasted with. Both cannot be true.

**Ruling: a platform freeze is never stood aside for a rollback.** One predicate, `rollbackExemptible`, is consulted by both seams; D7 is unchanged at the org tier, where the org owns both sides of the "broken release vs change window" trade.

`overridable` is deliberately **not** consulted here. It reads as the natural mapping and it is wrong: `overridable` admits a *reasoned* override by an actor holding `freeze:override` at the org root — two independent authorities, both required, and the reason lands on the Decision. The rollback path checks none of that, so mapping one bit onto both would mean an operator who admitted a narrow, audited, high-privilege escape had silently also admitted an unaudited one at `object:write`. One bit, one meaning.

*Stated consequence, the one place this narrows D7:* a rollback wave covered by **both** tiers is blocked whole, rather than admitted for the targets only an org freeze covers. Chosen over the finer alternative (resolve the tiers separately for a rollback and hold only the platform-covered targets) because the finer one needs a second resolution pass that `frozenTargets`, `partiallyFrozen` and the hold Decision would each have to agree with — a second source of truth about what is frozen, which is exactly what `freezesByTarget` exists to be the only one of. A wave containing a platform-frozen target cannot complete this window either way. **The operator's remedy is the one the block sentence already names:** `PUT`/`DELETE /v1/instance/freezes/{key}` shortens or retracts the freeze over the operator token.

### 9. `overridable: true` is reachable only for `allEnvironments` freezes (a limit, not a decision)

Two pre-existing facts compose into a sharper limit than §4 admits, and it is recorded here rather than left to be rediscovered:

1. `EvaluateWaveGateContext` carries no `overrideFreeze`, so the override loop runs **only** on `validating → accepted` (true at the org tier too, and not created by M25.3); and
2. `evaluateLifecycleGate` evaluates the change's **declared** targets — components/services — never its placements. A component is not a `deployment-target`, so `readStageCoordinate` returns `null` for it and `instanceFreezeCovers` is false for every addressing form except `allEnvironments`.

Therefore an **environment-addressed** platform freeze never matches on the only edge an override can be supplied on, and setting `overridable: true` on one buys nothing. Both halves are pinned by a test (case E2) that blocks the same freeze at the wave boundary as its control, so the limit changes loudly rather than lapsing quietly. **This is an open question for the owner**, not a settled ruling: closing it means either giving the wave boundary an override path, or expanding a component target to its placements at the accept edge — the second is a real tightening (any change bound for a frozen stage becomes un-acceptable) and is not a decision an implementer should take.

Two earlier tests proved §4's ruling in a configuration production cannot produce (they passed a *placement* id on the `lifecycle_edge` path); they now use the declared-target shape the real caller supplies, and the quantifier case additionally exercises the sharp principal — an actor holding `freeze:override` at the component only, who satisfies the org freeze and fails the platform check at the org root.

### 10. Operator match values are trimmed where they enter

`readStageCoordinate` trims what the graph declares and `instanceFreezeCovers` compares with `!==`, so an operator value stored untrimmed matched **nothing** while `PUT` returned 200 and `GET /v1/instance/freezes` listed the row cleanly — a freeze that reads as in force and holds nothing, which is the one failure mode this tier must not have. 0086's `instance_freezes_match_ck` cannot close it: `length(btrim(...)) > 0` *tests* a value, it does not *store* one. `InstanceFreezeMatchSchema` now trims `environment` and `region` before `min(1)`, so an all-whitespace value is a 400 naming the addressing rule. No emitted JSON Schema changes (`{"type":"string","minLength":1}` either way), verified against the generated document rather than inferred.

*Sibling with the same property, observed and deliberately not changed:* `coordination/regional-executors.ts`'s `listRegionTargets` compares an API-supplied environment against the **untrimmed** `properties ->> 'environment'`, while `readStageProperties` beside it trims. That is a shipped read-only route whose two sides are at least consistent with each other; it is flagged rather than fixed here because changing it changes an existing surface's behaviour with no test evidence that anyone relies on either reading.
...
* **An admitted-overridable platform freeze is exercisable only on the lifecycle `accept` edge**, because `evaluateWaveGate` passes no `overrideFreeze`. Pre-existing and not created here, but it means `overridable: true` does not unblock a change already `executing` — and, composed with the accept edge's target shape, only for `allEnvironments` freezes at all (§9).
* **A rollback does not walk past a platform freeze** (§8). D7 is bounded at the tier boundary rather than deleted; it is unchanged at the org tier.
...
* **Whether `overridable: true` should be reachable for an environment-addressed freeze** (§9) — an owner decision, with two possible closures and one of them a real tightening.
