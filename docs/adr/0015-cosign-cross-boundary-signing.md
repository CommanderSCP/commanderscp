...
- **cosign today is release-only.** The only cosign in the tree is `deploy/airgap/src/cosign.ts`, which signs the air-gap *release* bundle (per-image digests + `CHECKSUMS.txt` via `cosign sign-blob`). It is already **keyful and offline** — `--tlog-upload=false` + `--insecure-ignore-tlog=true`, **no Fulcio, no Rekor**. There is **zero** cosign in `apps/server` or `packages`.
...
| **cosign (supply-chain)** | External, standard-tool **artifact provenance** | Every cross-boundary artifact (all types) **+** the SCP promotion manifest | Vendored cosign binary, keyful/offline | **New (this ADR, lands M17.3/M17.4)** |
...
- **`type: 'blob'`** (rpm / deb / npm / config bundle / infra plan / SBOM) → verified with `cosign verify-blob` against a **detached** signature.
...
### 3. Implementation — vendored cosign binary, keyful/offline, shared module

- A **vendored cosign binary** is invoked **in-process** (subprocess), reusing the proven `deploy/airgap/src/cosign.ts` keyful/offline pattern: `--tlog-upload=false`, no Fulcio/Rekor, no network. cosign is **already vendored** for release signing, so the binary and offline behavior are proven.
...
- The **executor** (e.g. an Argo Workflows cosign step) signs artifacts **at build**, in the build system, with the build system's credentials. SCP does not run that step.
- **SCP verifies** origin signatures — a **gate control**, sibling of the M17.1 scan control (ADR-0016) — and **signs its own promotion manifest** (an authorization attestation over the digests, exactly as it Ed25519-signs journals).
- **SCP never `cosign sign`s an origin artifact.** It verifies what the executor produced and vouches (over its own manifest) for the set it authorizes to cross. Signing its own authorization record is coordination, not execution — identical in kind to the existing Ed25519 journal signing.

### 6. Verify at every hop (ADR-0011 / ADR-0013)

M17.4 adds, in `import-repo.ts` **after** the existing Ed25519 gate, a fail-closed step that (a) `cosign verify`/`verify-blob`s each artifact's signature against the distributed pubkey, and (b) asserts the arrived artifact-set **exactly equals** the cosign-signed manifest (nothing added or substituted). Retrans is naming-only/unbuilt today; M17.4 lands the **outpost-side** verify (ties to M15.2) and, where retrans exists, the CDS-boundary verify. Retrans-as-a-real-component is its own work.
...
- **Air-gap first (principle 5):** OK — keyful `--tlog-upload=false` (precedent: `deploy/airgap`); the pubkey travels via federation config, verification is fully offline, no transparency-log or network fetch.
...
**Costs / honesty**
- The worker sign path takes a vendored-binary dependency (mitigated: already vendored, proven offline).
