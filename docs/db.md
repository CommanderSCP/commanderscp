...
UPDATED (team-pipeline-iac increment 0): CHECK constraints now get a narrower version of the same treatment. `pipeline_evidence_source_check` drifted — migration 0107 widened the live constraint by hand and schema.ts was never updated — and nothing here or in `snapshot-freshness.test.ts` caught it, because neither compared expressions at all. A new `describe` block below the index one compares, for every check constraint BOTH sides name, the SET of literal values each allows (not the rendered SQL text: Postgres rewrites `IN (...)` to `= ANY (ARRAY[...])` with per-value casts, so a textual diff would false-positive on every constraint that exists). Unlike the index gate, this is not bidirectional-and-complete: `schema.ts` writes `check(...)` for only 5 of the 23 CHECK constraints the migrated database holds, so a "declares every one, by name" assertion (the index gate's shape) would fail today on 18 pre-existing, out-of-scope constraints across 7 modelled tables that were simply never given a `check()` — `instance_freezes`'s own doc comment says "(DB CHECK)" for one of them, an unswept hazard comment. That backfill is a separate piece of work; this gate only guarantees that a constraint schema.ts DOES claim never silently disagrees with the one Postgres actually enforces.
