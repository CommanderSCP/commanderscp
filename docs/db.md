...
UPDATED (team-pipeline-iac increment 0): CHECK constraints got a narrower version of the same treatment at first. `pipeline_evidence_source_check` drifted — migration 0107 widened the live constraint by hand and schema.ts was never updated — and nothing here or in `snapshot-freshness.test.ts` caught it, because neither compared expressions at all. A `describe` block below the index one compares, for every check constraint BOTH sides name, the SET of literal values each allows (not the rendered SQL text: Postgres rewrites `IN (...)` to `= ANY (ARRAY[...])` with per-value casts, so a textual diff would false-positive on every constraint that exists). At that point it was not bidirectional-and-complete: `schema.ts` wrote `check(...)` for only 5 of the 23 CHECK constraints the migrated database holds, so a "declares every one, by name" assertion (the index gate's shape) would have failed on 18 pre-existing, out-of-scope constraints across 7 modelled tables that were simply never given a `check()` — `instance_freezes`'s own doc comment said "(DB CHECK)" for one of them, an unswept hazard comment.

CLOSED (db-check-constraint-coverage increment): that backfill is done. All 15 of the 18 undeclared constraints that live on MODELED tables now have a `check()` in schema.ts, matching the migrated database's name and allowed-values set exactly — `bundle_transfers_channel_check`, `governance_move_rungs_tier_ck`, `governance_move_instance_rung_singleton_ck`, `instance_freezes_key_ck`/`_reason_ck`/`_window_ck`/`_match_ck`, `scan_exclusion_admissions_tier_ck`/`_class_ck`/`_origin_ck`, `scan_requirement_floors_tier_ck`/`_origin_ck`/`_nonneg_ck`, and `source_mappings_scope_check`/`_journey_kind_check`. Migration `0114_check_constraint_schema_backfill.sql` is a documented database no-op — the constraints already existed, created by their own original migrations (0029, 0074, 0082, 0083, 0086, 0087, 0112) — that exists only so `drizzle-kit generate` stops believing they are new the moment schema.ts declares them (verified: `drizzle-kit generate` reports "No schema changes, nothing to migrate" after 0114). A new `it` above the value-agreement one now asserts full name coverage for check constraints, mirroring the index gate's "declares every index the database holds" shape, filtered by the same `UNMODELLED_TABLES` set (`scanner_assignments`, `scan_db_staleness_policy`, `dependency_subscription_unlock`, `objects_m0_deprecated`) that the index gate's absence of indexes on those tables makes moot. The remaining 3 constraints (`scan_db_staleness_policy_singleton_ck`/`_nonneg_ck`, `dependency_subscription_unlock_singleton_ck`) stay undeclared for the same reason those two tables are outside the index gate: there is no `pgTable` to hang a `check()` on. Mutation-proven: removing `governance_move_instance_rung_singleton_ck`'s `check()` from schema.ts turns the name-coverage test red naming it; no other test moves.

NOT COVERED HERE: grants and RLS policies. Those have no schema.ts counterpart to compare against for any table, so a gate over them would assert the absence of something schema.ts never claimed.
...
SERVER-WRITTEN ONLY, and that is the whole point of it being a column. It lived in `labels` until 0068, where the prune TARGET could rewrite it under plain `object:write` — enrolling an arbitrary object into a stack's delete pool, or walking its own object out of one. The one module that writes it is `coordination-as-code/stack-ownership.ts`, with two writers: STAMP, called from the IaC apply path, and RELEASE (NULL only), called from the audited `POST /stacks/{stackName}/release` door, which requires the authority to decommission the whole stack (docs/coordination-as-code.md §328–§331). No request body can SET it to a value and no route passes one, exactly as for `origin_domain_id`, `provenance` and `domain_local`. (The 0068 `COMMENT ON COLUMN` still says "sole writer … called from the IaC apply path"; a comment-only migration was not worth a migration number.)
...
ONE EXCEPTION (team-pipeline-iac increment 0): check constraint EXPRESSIONS, not just names. `pipeline_evidence_source_check` was a name both sides agreed on while disagreeing about which values it allowed — the snapshot had been reconciled from a schema.ts that was itself stale relative to migration 0107, so the two files matched each other and were still wrong. A name-only comparison would have stayed green. The comparison renders both sides' `sql` the same way drizzle-kit does (`PgDialect().sqlToQuery`), so it is exactly what the next `db:generate` would diff.
