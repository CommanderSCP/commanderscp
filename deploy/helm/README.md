...
**Every pod template this chart renders declares a `resources` block** (`api.resources`,
`worker.resources`, `migrations.resources`, `postgres.evalInCluster.resources`) — an absent one
means BestEffort QoS, which the kubelet evicts first under node memory pressure, and is exactly how
the eval Postgres was the first pod killed in the 2026-09-17 incident; `tools/helm-verify` now
asserts the property holds for every container in every rendered template, not just this one.
...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `federationRole`                   | `SCP_FEDERATION_ROLE`                                                                                                                                    | `commander` (default) / `outpost` / `retrans`, always rendered. Unset at the process = UNDECLARED: promotion export, cosign key minting and dependency automation refuse fail-closed, and boot logs one warning (§8.9).                                                                                                                                                                                                                                                                                                                                              |
...
| `stackd.*`                         | a separate `scp-stackd` Deployment + its own ServiceAccount/RBAC; `SCP_OPERATOR_DATABASE_PASSWORD` on api/worker when no operator connection is supplied | The Standard Stack controller (ADR-0058). **Off at the chart's own default** — `scp install` turns it on explicitly for every role but `retrans` (ADR-0060 §3, #422 review); a bare `helm install` stays off. Creates the five backend namespaces (kept on uninstall). Its credential and the `scp_operator` login are generated and provisioned by the migrations Job by default — see "GitOps / Argo CD" below for `stackd.existingCredentialSecret` / `operatorApi.databaseUrlSecret`, the pre-provisioned alternative a `lookup`-blind renderer (Argo CD) needs. |
| `internalBaseUrl`                  | `SCP_INTERNAL_BASE_URL`                                                                                                                                  | How this instance calls ITSELF (UI SSR, boot-time demo seed). Not human-facing — see `publicBaseUrl`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `publicBaseUrl`                    | `SCP_PUBLIC_BASE_URL`                                                                                                                                    | How this instance names itself to a human — the CLI device-login `verificationUri`. Unset ⇒ a relative `/device` path.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `postgres.evalInCluster.resources` | _(pod `resources`, not an env var)_                                                                                                                      | Eval-only in-cluster Postgres request/limit block. Defaults to `requests: {cpu: 50m, memory: 256Mi}`, no limit — a container with NO resources at all is BestEffort QoS and the kubelet's first eviction candidate under node memory pressure (the 2026-09-17 incident this default exists to fix). `api.resources`/`worker.resources`/`migrations.resources` already carried a non-empty default before this incident and are unaffected.                                                                                                                           |
...
### GitOps / Argo CD — the `existingSecret` overrides (#422 adversarial review, LIVE RISK)

**The problem, measured directly:** this chart generates several values with Helm's
`lookup`-and-preserve pattern (`secrets-generated.yaml`, `stackd.yaml`) — `lookup` queries the live
cluster for a Secret it already created, and if one exists, REUSES its value instead of generating
a new one. Argo CD's repo-server renders every sync with `helm template`, which has **no live
cluster context at all** — `lookup` always returns nothing there, so a `stackd.enabled: true`
release tracked by Argo CD (with `selfHeal`) regenerates the stack controller's credential AND
`scp_operator`'s database password **on every single sync**. The credential rotation is mostly
self-healing (ADR-0058's rotation path revokes the old one by id), but the password is not:
`apps/server/src/db/provision.ts` REFUSES to reset `scp_operator`'s live password when it differs
from what the connection string says ("refusing to reset the password for role scp_operator") — so
the very next migrations Job PreSync hook fails, and every sync after it too, until someone
intervenes by hand.
...
`secretKeyRef` names the pre-provisioned Secret, not the chart's own — the gate this whole section
exists to keep true.
