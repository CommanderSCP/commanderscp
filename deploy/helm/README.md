...
| `stackd.*`                         | a separate `scp-stackd` Deployment + its own ServiceAccount/RBAC; `SCP_OPERATOR_DATABASE_PASSWORD` on api/worker when no operator connection is supplied | The Standard Stack controller (ADR-0058). Off until M29.1's installer turns it on. Creates the five backend namespaces (kept on uninstall). Its credential and the `scp_operator` login are generated and provisioned by the migrations Job — nothing to create by hand. |
