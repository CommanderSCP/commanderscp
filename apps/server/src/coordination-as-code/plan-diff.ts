...
/** Pure desired-vs-actual diff engine for `@scp/coordination-as-code` plans. See docs/coordination-as-code.md §58. */

export const MANAGED_BY_LABEL = "scp:managed-by";
export const STACK_LABEL = "scp:stack";
export const MANAGED_BY_IAC_VALUE = "iac";

/** The human-readable mirror of stack ownership, on each row. See docs/coordination-as-code.md §59. */
...
/** True if this row is owned by THIS stack. See docs/coordination-as-code.md §60. */
/** Stack theft: a manifest claiming another stack's object. See docs/coordination-as-code.md §61. */
...
/** A manifest object whose domain is already resolved. See docs/coordination-as-code.md §62. */
...
/** A source-mapping entry with every optional field normalized. See docs/coordination-as-code.md §63. */
...
/** A declared pipeline hook. See docs/coordination-as-code.md §66. */
...
export interface ResolvedManifest {
  ...
  /** ABSENT AND EMPTY ARE THE SAME THING HERE, deliberately. See docs/coordination-as-code.md §67. */
  ...
  /** HERE, AND ONLY HERE, ABSENT AND EMPTY ARE DIFFERENT. See docs/coordination-as-code.md §68. */
  producers: ResolvedManifestDependencyProducer[] | null;
  /** The second nullable collection, null for the same reason. See docs/coordination-as-code.md §69. */
  governanceMoveRungs: string[] | null;
  /** The third nullable collection, null for the same reason. See docs/coordination-as-code.md §70. */
  pipelineHooks: ResolvedManifestPipelineHook[] | null;
  /** The two collections that were authorable and dropped. See docs/coordination-as-code.md §71. */
...
export interface PlanDiffSnapshot {
  /** Live objects the diff must reason about, keyed by urn. See docs/coordination-as-code.md §72. */
  existingObjects: ExistingObjectSnapshot[];
  /** Live relationship triples this stack manages. See docs/coordination-as-code.md §73. */
  managedRelationships: ExistingRelationshipTriple[];
  /** Live relationship triples that exist for any reason. See docs/coordination-as-code.md §74. */
  existingRelationships: ExistingRelationshipTriple[];
  /** Live source mappings hanging off objects this stack owns. See docs/coordination-as-code.md §75. */
  ...
  /** Live declarations for the coordinates this manifest names. See docs/coordination-as-code.md §76. */
  existingDependencyProducers: ResolvedManifestDependencyProducer[];
  /** Live move rungs whose subject container this stack owns. See docs/coordination-as-code.md §77. */
  managedGovernanceMoveRungs: string[];
  /** Live `pipeline_hooks` rows whose COMPONENT this stack owns. See docs/coordination-as-code.md §78. */
...
/** A hook's DIFF key. See docs/coordination-as-code.md §79. */
...
/** The ref pattern is in the key and the other is deliberately out. See docs/coordination-as-code.md §80. */
/** A producer declaration's identity. See docs/coordination-as-code.md §81. */
...
/** The stand-in urn when a producer object cannot be named. See docs/coordination-as-code.md §82. */
...
/** A binding's identity. See docs/coordination-as-code.md §83. */
...
/** The fields whose drift makes a binding an `update`. See docs/coordination-as-code.md §84. */
...
/** Computes the create/update/delete/noop diff for one `@scp/coordination-as-code` plan. See docs/coordination-as-code.md §85. */
export function computePlanDiff(manifest: ResolvedManifest, snapshot: PlanDiffSnapshot): PlanDiff {
  ...
  // Prune: objects this stack managed that the manifest dropped. See docs/coordination-as-code.md §86.
  ...
  for (const mapping of manifest.sourceMappings) {
    ...
    // §10.6 — the ONE in-place convergence this diff performs on a mapping. See docs/coordination-as-code.md §87.
  ...
  for (const binding of manifest.executorBindings) {
    ...
    // Two declarations for the same. See docs/coordination-as-code.md §88.
  ...
  // DEPENDENCY-LINE PRODUCERS (ADR-0032 §7e). See docs/coordination-as-code.md §89.
  ...
  // `governance:move` RUNGS (ADR-0038 §2; proposal §9.6 Q4). See docs/coordination-as-code.md §90.
  ...
  // PIPELINE HOOKS (D11/D21; migration 0096). See docs/coordination-as-code.md §91.
  ...
  // ROLLOUTS (D12) and CONVERGENCE (D25(b)). See docs/coordination-as-code.md §92.
  ...
  // ROLE BINDINGS (drizzle/0108). See docs/coordination-as-code.md §93.
...
/** Components this plan creates with no incoming containment. See docs/coordination-as-code.md §94. */
/** Readable descriptions of the entries this plan may not write. See docs/coordination-as-code.md §95. */
export function duplicateProjectionDeclarations(manifest: ResolvedManifest): string[] {
  ...
  // A coordinate declared twice. See docs/coordination-as-code.md §96.
  ...
  // A hook is keyed on its identity here, not its declaration. See docs/coordination-as-code.md §97.
...
/** Readable descriptions of producer declarations it may not make. See docs/coordination-as-code.md §98. */
...
/** Readable descriptions of move rungs this plan may not write. See docs/coordination-as-code.md §99. */
...
/** Readable descriptions of the entries this plan may not touch. See docs/coordination-as-code.md §100. */
export function unownedProjectionDeclarations(diff: PlanDiff): string[] {
  ...
  for (const binding of diff.executorBindings ?? []) {
    ...
    // The pair must ALSO survive this plan. See docs/coordination-as-code.md §101.
    if (
  ...
  // PIPELINE HOOKS, under the identical rule and for the identical reason. See docs/coordination-as-code.md §102.
