...
/** Rejects a plan that would write such a row it may not. See docs/coordination-as-code.md §104. */
...
/** Rejects a plan whose producer declarations it may not make. See docs/coordination-as-code.md §105. */
/** Rejects a plan whose move-rung declarations it may not make. See docs/coordination-as-code.md §106. */
...
/** Rejects a manifest declaring the same thing twice. See docs/coordination-as-code.md §107. */
/** A manifest hook, flattened to the row shape the diff keys on. See docs/coordination-as-code.md §108. */
...
/** Runs the same three checks for every inline binding. See docs/coordination-as-code.md §109. */
...
/** The thin database wrapper around the pure diff engine. See docs/coordination-as-code.md §110. */
...
/** The URN of one object by id, TOMBSTONES INCLUDED. See docs/coordination-as-code.md §111. */
...
/** Live objects this stack OWNS. See docs/coordination-as-code.md §112. */
...
/** Bindings THIS stack owns. See docs/coordination-as-code.md §113. */
...
/** Assembles a snapshot from live state and runs the diff. See docs/coordination-as-code.md §114. */
export async function computeDiffForManifest(
  ...
  // `governance:move` RUNG SUBJECTS. See docs/coordination-as-code.md §115.
  ...
  // A role binding names TWO objects and OWNS NEITHER. See docs/coordination-as-code.md §116.
  ...
  // C1 — the ownership pool for `source_mappings`/`executor_bindings`. See docs/coordination-as-code.md §117.
  ...
  // THE BINDING POOL SPANS OBJECTS *AND* PLACEMENTS. See docs/coordination-as-code.md §118.
  ...
  // PRODUCER DECLARATIONS (ADR-0032 §7e). See docs/coordination-as-code.md §119.
  ...
  if (manifest.producers !== undefined) {
    ...
    // THE PRUNE POOL. See docs/coordination-as-code.md §120.
    ...
    // THE EXISTENCE POOL. See docs/coordination-as-code.md §121.
  ...
  // `governance:move` RUNGS (ADR-0038 §2). See docs/coordination-as-code.md §122.
  ...
  // PIPELINE HOOKS (D11/D21; migration 0096). See docs/coordination-as-code.md §123.
  ...
  // ROLE BINDINGS AND ORG ROLES. See docs/coordination-as-code.md §124.
  ...
  // §9 — STACK THEFT IS A 409, NOT AN INTERNAL ERROR. See docs/coordination-as-code.md §125.
...
/** The apply-time half of the stack-theft refusal. See docs/coordination-as-code.md §126. */
...
/** The diff, with its one typed refusal shaped for HTTP. See docs/coordination-as-code.md §127. */
...
/** Loads and locks a plan for apply, rejecting non-pending. See docs/coordination-as-code.md §128. */
...
// Apply: per-entry authorization-scope resolution, then mutation execution. See docs/coordination-as-code.md §129.
...
/** Resolves which permission and scope each entry needs. See docs/coordination-as-code.md §131. */
export async function prepareApplyChecks(
  tx: TenantTx,
  orgId: string,
  actorObjectId: string,
  diff: PlanDiff,
  /** The stack this diff belongs to, required not optional. See docs/coordination-as-code.md §132. */
  stackName: string
  ...
  assertComponentsContained(diff);
  // §9 STACK THEFT, RE-CHECKED AT APPLY AGAINST LIVE OWNERSHIP. See docs/coordination-as-code.md §133.
  await assertNoStackTheftAtApply(tx, orgId, diff, stackName);
  // Those two invariants get the same defence in depth. See docs/coordination-as-code.md §134.
  assertProjectionsOwned(diff);
  assertProducerDeclarationsValid(diff);
  assertGovernanceMoveRungsValid(diff);
  assertInlineBindingsValid(diff);
  ...
  for (const entry of diff.objects) {
    // A PAIR-BOUND type. See docs/coordination-as-code.md §135.
    ...
    // M25.7 — A PROJECTION-BOUND type. See docs/coordination-as-code.md §136.
    ...
    if (entry.action === "create") {
      ...
      // M5 (BUILD_AND_TEST.md §8 M5 security note). See docs/coordination-as-code.md §137.
    ...
    if (entry.action !== "noop") {
      ...
      // A containment move is a write at two places. See docs/coordination-as-code.md §138.
      ...
      if (entry.action === "update" && destination && destination !== found.domainId) {
        ...
        // THE `governance:move` TWIN. See docs/coordination-as-code.md §139.
  ...
  // ROLE BINDING ENDPOINTS. See docs/coordination-as-code.md §140.
  ...
  for (const entry of diff.relationships) {
    // RESOLVED FOR EVERY NON-DELETE ENTRY, INCLUDING `noop`. See docs/coordination-as-code.md §141.
    await resolveEndpoint(entry.fromUrn);
    await resolveEndpoint(entry.toUrn);
    if (entry.action === "noop") continue;
    // M5 CRITICAL (adversarial review). See docs/coordination-as-code.md §142.
    ...
    // THE `governance:move` TWIN FOR ROUTE 2. See docs/coordination-as-code.md §143.
  ...
  // PIPELINE HOOKS (D11/D21). See docs/coordination-as-code.md §144.
  ...
  // ROLLOUTS (D12) and CONVERGENCE (D25(b)). See docs/coordination-as-code.md §145.
  ...
  for (const entry of diff.placements ?? []) {
    ...
    const component = await resolveEndpoint(entry.componentUrn);
    checks.push({ permission: "object:write", scopeObjectId: component.scopeObjectId });
    // RESOLVED BUT NOT CHECKED. See docs/coordination-as-code.md §146.
    await resolveEndpoint(entry.deploymentTargetUrn);
  }

  // Producer declarations: policy write at the org root. See docs/coordination-as-code.md §147.
  ...
  // Move rungs: policy write at or above the subject, per entry. See docs/coordination-as-code.md §148.
  ...
  for (const entry of diff.executorBindings ?? []) {
    if (entry.action === "noop") continue;
    // A placement-targeted binding authorizes at the component. See docs/coordination-as-code.md §149.
    ...
    // A system-backed binding dispatches with that system's token. See docs/coordination-as-code.md §150.
...
/** Executes an authorized diff inside the caller's transaction. See docs/coordination-as-code.md §151. */
export async function executePlanDiff(
  ...
  // OWNERSHIP, STAMPED FOR EVERY OBJECT THIS MANIFEST DECLARES. See docs/coordination-as-code.md §152.
  ...
  // The relationship half of the same stamp, after the creates. See docs/coordination-as-code.md §153.
  ...
  // C1 — projection rows. See docs/coordination-as-code.md §154.
  ...
  /** The binding target a diff entry names, either way. See docs/coordination-as-code.md §155. */
  ...
  for (const entry of diff.placements ?? []) {
    ...
    // DECISION Q2 — REFUSE, naming the binding. See docs/coordination-as-code.md §156.
  ...
  // Placement creates run first, because a binding may need one. See docs/coordination-as-code.md §157.
  for (const entry of diff.placements ?? []) {
    if (entry.action !== "create") continue;
  ...
  // PIPELINE HOOKS (D11/D21; migration 0096). See docs/coordination-as-code.md §159.
  for (const entry of diff.pipelineHooks ?? []) {
    if (entry.action !== "delete") continue;
  ...
  // ROLLOUTS AND CONVERGENCE. See docs/coordination-as-code.md §160.
  for (const entry of diff.rollouts ?? []) {
    if (entry.action !== "delete") continue;
  ...
  // ROLE BINDINGS AND ORG ROLES. See docs/coordination-as-code.md §161.
  for (const entry of diff.roleBindings ?? []) {
    if (entry.action !== "delete") continue;
  ...
  // PRODUCER DECLARATIONS (ADR-0032 §7e). See docs/coordination-as-code.md §162.

  /** The coordinate must still be held by whoever the plan said. See docs/coordination-as-code.md §163. */
  ...
  // The move rungs, and the permission each entry needs. See docs/coordination-as-code.md §164.
  for (const entry of diff.governanceMoveRungs ?? []) {
    if (entry.action !== "delete") continue;
