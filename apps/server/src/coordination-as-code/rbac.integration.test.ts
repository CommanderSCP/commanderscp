...
import { OrgRole, RoleBinding, Stack } from "@scp/coordination-as-code";
import { withTenantTx } from "../db/tenant-tx.js";
import { hasPermission } from "../authz/resolve.js";
import { createStackManagedRoleBinding } from "./rbac-apply.js";
import {
  buildTestServer,
  createTestOrg,
  createTestUser,
...
/** IaC-DECLARED ROLES AND ROLE BINDINGS. See docs/coordination-as-code.md §47. */
describe("IaC: roles and role bindings apply, prune, and respect the doors", () => {
  ...
  it("REFUSES an apply whose principal lacks what it is granting (the subset rule, on the IaC path)", async () => {
    // THE GAP THIS CLOSES, found by mutation. See docs/coordination-as-code.md §48.
...
describe("IaC: the RBAC doors are not bypassed by the apply path", () => {
  ...
  it("REFUSES a manifest binding to a GROUP, even though the construct also refuses it", async () => {
    ...
    // that never went through `@scp/coordination-as-code` at all. Both doors exist because both paths do.
  ...
  it("names WHICH HALF of a binding it could not resolve — the guard is live, not dead code", async () => {
    // Entered at the writer rather than the route deliberately. See docs/coordination-as-code.md §49.
