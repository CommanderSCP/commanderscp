...
/** A declared pipeline hook (D11/D21, migration 0096). See docs/iac/plan-diff.md §1. */
...
export interface ResolvedManifest {
  ...
  /** HERE, AND ONLY HERE, ABSENT AND EMPTY ARE DIFFERENT. See docs/iac/plan-diff.md §2. */
...
/** The fields whose drift makes a binding an `update`. See docs/iac/plan-diff.md §3. */
...
export function computePlanDiff(manifest: ResolvedManifest, snapshot: PlanDiffSnapshot): PlanDiff {
  ...
  // `governance:move` RUNGS (ADR-0038 §2; proposal §9.6 Q4). See docs/iac/plan-diff.md §4.
...
export function unownedProjectionDeclarations(diff: PlanDiff): string[] {
  ...
  for (const binding of diff.executorBindings ?? []) {
    ...
    // The pair must ALSO survive this plan. See docs/iac/plan-diff.md §5.
    if (
