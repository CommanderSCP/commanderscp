...
/** THE IaC DOOR ONTO A CAMPAIGN'S DEADLINE. See docs/iac.md §6. */
describe("IaC apply cannot widen a campaign's deadline at plain object:write", () => {
  ...
  /** A campaign with one target and a deadline, plus the stack. See docs/iac.md §7. */
  ...
  it("R3: the SAME Operator may SHORTEN it, and may restate it unchanged — the gate is about direction", async () => {
    ...
    // ---- RESTATE THE SAME INSTANT IN A DIFFERENT RENDERING. See docs/iac.md §8.
    ...
    // And now the other direction, on the same campaign. See docs/iac.md §9.
  ...
  it("R4: an OWNER applies the same widening manifest and it lands — the bar is a permission, not a ban on the door", async () => {
    ...
    // The bootstrap admin is an owner, which is where it sits. See docs/iac.md §10.
  ...
  it("R5: a campaign with NO deadline is applied by the Operator freely — the guard is a delta, not a key-presence test", async () => {
    ...
    // The case that makes the flat rule untenable. See docs/iac.md §11.
  ...
  it("R6: replacing a readable deadline with an UNREADABLE document is a widening too", async () => {
    ...
    // The resolver calls this malformed, and the loop then. See docs/iac.md §12.
    const applied = await planAndApply(operator.token, {
      stackName: f.stackName,
      objects: [
        {
  ...
  /** R1-R6 above closed the deadline INSTANT at this door. See docs/iac.md §13. */

  /** A fully-formed waiver, minted the way a MANIFEST mints one. See docs/iac.md §14. */
  ...
  it("W3: RESTATING the waivers already in force is free at `object:write` — the delta is empty", async () => {
    ...
    // MINTED THROUGH THE TYPED DOOR, by the Owner. Two things at once. See docs/iac.md §15.
    ...
    // ---- RE-APPLY THE IDENTICAL WAIVERS. NOT a byte-identical manifest. See docs/iac.md §16.
  ...
  it("W5: REMOVING a waiver is a tightening and stays at `object:write`", async () => {
    ...
    // The deliberate divergence from the sibling four lines up. See docs/iac.md §17.
