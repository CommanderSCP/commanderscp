...
/** M25.9 / OWNER RULING D1. See docs/routes/governance.md §1. */
...
export function registerGovernanceRoutes(app: FastifyInstance, deps: AppDeps): void {
    ...
    handler: async (request, reply) => {
      ...
      reply.status(200).send({
        items: runs.map((r) => ({
          ...
          // M22.8 — WHICH CROSSING THIS RUN AUTHORIZED. See docs/routes/governance.md §2.
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const freeze = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        // M25.7 / OWNER DECISION D6. See docs/routes/governance.md §3.
  ...
  // M25.1 — LIFT AND SHORTEN. See docs/routes/governance.md §4.
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const lifted = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        // M25.7 — THE LIFT MUST REACH DOWNSTREAM TOO. See docs/routes/governance.md §5.
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const updated = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        // M25.9 / OWNER RULING D1. See docs/routes/governance.md §6.
