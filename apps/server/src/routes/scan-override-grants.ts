...
export function registerScanOverrideGrantRoutes(app: FastifyInstance, deps: AppDeps): void {
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const grant = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        // ...AND IT MUST BE AN ANCESTOR. See docs/routes/scan-override-grants.md §1.
  ...
  const decide = async (
    ...
    return withTenantTx(deps.db, auth.orgId, async (tx) => {
      ...
      // SEPARATION OF DUTIES. See docs/routes/scan-override-grants.md §2.
      ...
      const updated = await updateObject(tx, {
        ...
        // THE ONE CALLER THAT MAY WRITE A DECISION. See docs/routes/scan-override-grants.md §3.
