...
/** `/federation` (DESIGN.md §13, BUILD_AND_TEST.md §8 M6). See docs/routes/federation.md §1. */
export function registerFederationRoutes(app: FastifyInstance, deps: AppDeps): void {
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const self = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        // THE RETRANS DOOR. See docs/routes/federation.md §2.
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const peer = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        // THE SECOND BAR. See docs/routes/federation.md §3.
        await authorize(tx, {
          orgId: auth.orgId,
          subjectObjectId: auth.subjectObjectId,
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const peer = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        // THE FIVE TRANSPORT FIELDS, SPREAD EXPLICITLY. See docs/routes/federation.md §4.
  ...
  // M15.5(c) — the retrans validate-then-relay (ADR-0019 §2). See docs/routes/federation.md §5.
  typed.route({
    method: "POST",
  ...
  // FEDERATION AUDIT WITNESS. See docs/routes/federation.md §6.
  typed.route({
    method: "GET",
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      // WAKE — enqueue immediate ticks and return fast. See docs/routes/federation.md §7.
      let wokenSync = false;
  ...
  /** THE TWO OVERLAY DOORS ARE NOT FEDERATION DOORS. See docs/routes/federation.md §8. */
  typed.route({
    method: "POST",
