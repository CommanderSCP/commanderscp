...
export function registerCampaignRoutes(app: FastifyInstance, deps: AppDeps): void {
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const page = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        // THE GATE, AND THE ROW FILTER, IN ONE CALL. See docs/routes/campaigns.md §1.
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      const auth = await requireAuth(deps, request);
      const result = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        // RESOLVE, THEN SCOPE. See docs/routes/campaigns.md §2.
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const campaign = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        // THE SECOND BAR ON THE WIDENING ACTS. See docs/routes/campaigns.md §3.
