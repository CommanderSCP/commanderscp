...
export function registerChangeSourceRoutes(app: FastifyInstance, deps: AppDeps): void {
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      ...
      const eventId = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        // MAJOR #5 — dedupe redeliveries/replays. See docs/routes/change-sources.md §1.
  ...
  // Typed first-party report ingress (DESIGN §12 Mode 1). See docs/routes/change-sources.md §2.
  typed.route({
    method: "POST",
  ...
  typed.route({
    ...
    handler: async (request, reply) => {
      const auth = await requireAuth(deps, request);
      const mapping = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        // READ THE ROW, THEN BAR AT ITS COMPONENT. See docs/routes/change-sources.md §3.
  ...
  /** DELETE a source_mapping. See docs/routes/change-sources.md §4. */
  typed.route({
    method: "DELETE",
    url: "/api/v1/change-sources/:sourceKind/mappings",
    schema: {
