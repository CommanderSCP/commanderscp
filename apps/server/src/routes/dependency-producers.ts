...
/** Re-exported so the decision consumers keep working. See docs/routes.md §114. */
export { PRODUCER_DECISION_KIND } from "../dependencies/producer-declaration.js";

/** THE PRODUCER DECLARATION'S AUTHORING SURFACE. See docs/routes.md §115. */

/** Resolve the producer to a live, in-org component. See docs/routes.md §116. */
...
export function registerDependencyProducerRoutes(app: FastifyInstance, deps: AppDeps): void {
  ...
  // POST /dependencies/producers — DECLARE. See docs/routes.md §117.
  typed.route({
    method: "POST",
    ...
    handler: async (request, reply) => {
      ...
      const result = await withTenantTx(deps.db, auth.orgId, async (tx) => {
        ...
        if (dryRun) {
          return {
            // NO PROJECTED DECLARATION. See docs/routes.md §118.
            declaration: null,
  ...
  // POST /dependencies/producers/retract — the peer verb. See docs/routes.md §119.
  typed.route({
    method: "POST",
  ...
  // GET /dependencies/producers — the read. See docs/routes.md §120.
  typed.route({
    method: "GET",
    url: "/api/v1/dependencies/producers",
    schema: {
