...
async function applyEntry(
  ...
  switch (entry.entryKind) {
    ...
    case "policy_upsert": {
      ...
      // THE EVIDENCE RESOLVES ITSELF. See docs/federation/import-repo.md §1.
    ...
    case "object_tombstone": {
      ...
      // A TOMBSTONED FREEZE MUST STOP BLOCKING. See docs/federation/import-repo.md §2.
    ...
    case "relationship_upsert": {
      ...
      try {
        ...
        // Endpoints not yet replicated locally. See docs/federation/import-repo.md §3.
...
/** THE OPENING CLAUSE MUST MATCH THE CODE. See docs/federation/import-repo.md §4. */
...
/** THE ANCHOR CLAUSE. See docs/federation/import-repo.md §5. */
...
/** THE DIAGNOSTIC for a contiguity break. See docs/federation/import-repo.md §6. */
...
export async function importSyncBundle(
  ...
  /** §7.2.6 RESYNC ONLY. See docs/federation/import-repo.md §7. */
  forceOverwrite = false
  ...
  // DIVERGENCE RAIL 4. See docs/federation/import-repo.md §8.
  ...
  for (const entry of toApply) {
    ...
    if (entryMatchesScope(entry, peer.syncScope)) {
      ...
      // THE SECOND DROP CHOKEPOINT. See docs/federation/import-repo.md §9.
      const payload = entry.payload;
