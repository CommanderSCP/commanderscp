...
/** MAY THIS PROCESS RUN THE POLL? See docs/dependencies/version-poll.md §1. */
...
export async function buildLineWorkList(db: Db, orgId: string): Promise<LineWorkItem[]> {
  return withTenantTx(db, orgId, async (tx) => {
    ...
    const subscribed = await listSubscribedComponentLines(tx, orgId, {
      // A background tick has no human actor. See docs/dependencies/version-poll.md §2.
...
/** The Decision for one polled line. See docs/dependencies/version-poll.md §3. */
/**
...
export async function startDependencyVersionPollLoop(
  ...
  // AND IT SAYS SO WHEN IT ALLOWS, TOO. See docs/dependencies/version-poll.md §4.
  console.info(
