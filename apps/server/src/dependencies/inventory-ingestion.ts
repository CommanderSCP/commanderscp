...
export async function ingestComponentManifests(
  ...
  const prepared = await withTenantTx(db, orgId, async (tx) => {
    ...
    // KNOWN PATHS FROM THIS REPOSITORY ONLY. See docs/dependencies/inventory-ingestion.md §1.
    const known = [
      ...new Set(
  ...
  let reads = 0;
  /** WHEN THIS PASS LOOKED. See docs/dependencies/inventory-ingestion.md §2. */
  ...
  return withTenantTx(db, orgId, async (tx) => {
    ...
    /** THE ORDERING GUARD. See docs/dependencies/inventory-ingestion.md §3. */
    ...
    if (newestObservedAt > readAt.getTime()) {
      // NOTHING IS WRITTEN. See docs/dependencies/inventory-ingestion.md §4.
      return {
        componentObjectId: input.componentObjectId,
    ...
    const decision = await insertDecisionIfChanged(tx, {
      ...
      reasonTree: {
        ...
        // PATH AND REASON, NEVER THE DETAIL. See docs/dependencies/inventory-ingestion.md §5.
