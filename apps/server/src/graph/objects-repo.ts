...
export async function createObject(tx: TenantTx, input: CreateObjectInput): Promise<GraphObject> {
  ...
  if (!input.federationImport) {
    ...
    // THE RESERVED GOVERNANCE LABEL NAMESPACE. See docs/graph/objects-repo.md §1.
  ...
  // M20.1 (ADR-0031 §1). See docs/graph/objects-repo.md §2.
  ...
  // M20.7 (ADR-0031 §6c) — record WHY, not just whether. See docs/graph/objects-repo.md §3.
  const inheritedFrom =
...
export async function updateObject(tx: TenantTx, input: UpdateObjectInput): Promise<GraphObject> {
  ...
  if (!input.federationImport) {
    ...
    // OWNER RULING 2026-08-25. See docs/graph/objects-repo.md §4.
  ...
  if (!input.federationImport && nextDomainId !== null && nextDomainId !== existing.domainId) {
    // THE VALIDATION HALF OF THIS WRITE. See docs/graph/objects-repo.md §5.
  ...
  // CONTAINMENT ROUTE 1. See docs/graph/objects-repo.md §6.
  ...
  if (!input.federationImport && !row.domainLocal) {
    await appendJournalEntry(tx, {
      ...
      payload: {
        ...
        revision: nextRevision,
        version: nextVersion,
        // M20.2 (ADR-0031 §2). See docs/graph/objects-repo.md §7.
...
export async function upsertObjectByUrn(
  ...
  if (
    ...
    const nextVersion = existing.version + 1;
    // CONTAINMENT ROUTE 1, SECOND WRITE SITE. See docs/graph/objects-repo.md §8.
    const reachBefore =
      nextDomainId !== existing.domainId
...
export async function deleteObject(
  ...
  // THE ADMINISTRATOR FLOOR. See docs/graph/objects-repo.md §9.
  ...
  // CONTAINMENT ROUTE 3. See docs/graph/objects-repo.md §10.
  ...
  // CASCADE: an object's edges must not outlive the object. See docs/graph/objects-repo.md §11.
  if (!input.federationImport && !removedForeignShadow) {
    const self = await ensureFederationSelf(tx, input.orgId);
  ...
  // THE ADMINISTRATOR FLOOR. See docs/graph/objects-repo.md §12.
