...
export function extractHint(sourceKind: string, headers: unknown, payload: unknown): ExtractedHint {
  ...
  // Resolve the event NAME. See docs/coordination/webhook-processor.md §1.
  ...
  return {
    ...
    sbom: generic.sbom,
    // D23: same story, same one line. See docs/coordination/webhook-processor.md §2.
...
export async function processChangeSourceEvents(tx: TenantTx, orgId: string): Promise<void> {
  ...
  for (const row of rows) {
    ...
    // M21.5 THE PROVENANCE LOOP. See docs/coordination/webhook-processor.md §3.
    ...
    // ADR-0046 §2 — THE CONFIG-SOURCE TRIGGER. See docs/coordination/webhook-processor.md §4.
    ...
    try {
      ...
      await tx.transaction(async (inner) => {
        ...
        const { change } = await proposeChange(inner, {
          ...
          // WHO DECLARED IT. See docs/coordination/webhook-processor.md §5.
