...
/** TRIGGER RETRY BACKOFF. See docs/coordination/reconcile.md §1. */
...
async function reconcileExecutingChange(
  ...
  /** IS THIS CHANGE A ROLLBACK? See docs/coordination/reconcile.md §2. */
                ...
                ? {
                    ...
                    targets: previousWaveWithTargets.targets
                      ...
                      .map((t) => ({
                        ...
                        // THE DEPLOY INSTANT, AS DATA. See docs/coordination/reconcile.md §3.
  ...
  // THE STAGE-DEPENDENCY HOLD. See docs/coordination/reconcile.md §4.
  ...
  /** THE OTHER HALF OF THE HOLD'S DEPENDENCY SET. See docs/coordination/reconcile.md §5. */
  ...
  /** THE FREEZE HOLD. See docs/coordination/reconcile.md §6. */
  ...
  /** THE CONTINUOUS-TEST HOLD. See docs/coordination/reconcile.md §7. */
  ...
  /** THE ONE QUALIFIER ON D7'S ROLLBACK EXEMPTION. See docs/coordination/reconcile.md §8. */
  ...
  for (const target of activeWave.targets) {
    ...
    if (target.status === "pending" || target.status === "triggering") {
      ...
      // THE FREEZE HOLD. See docs/coordination/reconcile.md §9.
      ...
      // THE CONTINUOUS-TEST HOLD. See docs/coordination/reconcile.md §10.
        ...
        await withTenantTx(db, orgId, async (tx) => {
          ...
          await insertDecision(tx, {
            ...
            inputContext: {
              ...
              targetObjectId: target.targetObjectId,
              phase,
              // BOUNDED BEFORE IT BECOMES A ROW. See docs/coordination/reconcile.md §11.
  ...
  // TERMINALIZATION, IN TWO RULES RATHER THAN ONE. See docs/coordination/reconcile.md §12.
...
/** THE EXPLAINABILITY HALF OF THE FREEZE HOLD. See docs/coordination/reconcile.md §13. */
...
/** THE FAIL-OPEN, MADE VISIBLE. See docs/coordination/reconcile.md §14. */
...
async function triggerWaveTarget(
  ...
  try {
    ...
    const blocked = await withTenantTx(db, orgId, async (tx) => {
      // (0) IS THE TARGET OBJECT STILL THERE? See docs/coordination/reconcile.md §15.
    ...
    const claim = await withTenantTx(db, orgId, async (tx) => {
      ...
      // The executor-specific target id. See docs/coordination/reconcile.md §16.
      const resolution = await resolveBindingForTarget(tx, orgId, targetObjectId, type);
    ...
    try {
      ref = await client.trigger({
        ...
        idempotencyKey,
        // M25.4 — THE CHANNEL THAT WAS NEVER WIRED. See docs/coordination/reconcile.md §17.
...
/** One org's tick. See docs/coordination/reconcile.md §18. */
export async function reconcileOrgTick(
  ...
  // ADR-0046 §4 — THE DOMAIN-LOCAL BINDING RECONCILER. See docs/coordination/reconcile.md §19.
  try {
    await withTenantTx(db, orgId, (tx) =>
  ...
  // ADR-0046 §2 — DRAIN THE CONFIG-SOURCE SYNC QUEUE. See docs/coordination/reconcile.md §20.
  if (host) {
    try {
  ...
  // OUTPOST-RUN CONTINUOUS PROBES. See docs/coordination/reconcile.md §21.
  try {
