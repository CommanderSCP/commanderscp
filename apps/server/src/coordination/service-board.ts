...
async function latestChangeByComponent(
  ...
  // ARM 1 — the local observation. See docs/coordination/service-board.md §1.
...
export async function buildServiceBoard(
  ...
  // WHICH FREEZE IS ACTUALLY ON THIS ROW. See docs/coordination/service-board.md §2.
  const placementsByComponent = new Map<string, string[]>();
  for (const p of placements) {
  ...
  for (const component of components) {
    ...
    if (!latest || !changeId) {
      // Nothing found for this component. See docs/coordination/service-board.md §3.
      stable += 1;
      rows.push({
