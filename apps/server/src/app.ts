...
/** Builds (but does not start listening on) the Fastify app. See docs/app.md §1. */
export async function buildApp(
  ...
  const app = Fastify({
    ...
    // Global body ceiling (http-limits.ts). See docs/app.md §2.
  ...
  app.setErrorHandler((err, request, reply) => {
    ...
    // A FRAMEWORK-RAISED CLIENT ERROR KEEPS THE STATUS THE FRAMEWORK GAVE IT. See docs/app.md §3.
  ...
  // THE PRODUCER DECLARATION'S AUTHORING SURFACE. See docs/app.md §4.
