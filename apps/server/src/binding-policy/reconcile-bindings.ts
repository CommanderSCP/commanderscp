/** THE DOMAIN-LOCAL BINDING RECONCILER. See docs/binding-policy.md §4. */
...
/** One reconcile pass for one org. See docs/binding-policy.md §5. */
...
/** Remove reconciler-owned rows nothing declares any more. See docs/binding-policy.md §6. */
...
/** Executor-binding effects for every target in one call. See docs/binding-policy.md §7. */
async function gatherContributions(
  ...
  const matchedByTarget = await matchPoliciesForTargetsByTarget(tx, {
    orgId,
    targetObjectIds,
    // The reconciler has no acting user. See docs/binding-policy.md §8.
    actorObjectId: SYSTEM_ACTOR_ID
  });
...
async function listReleaseTypes(
  ...
  // Read DIRECTLY declared edges only. See docs/binding-policy.md §9.
