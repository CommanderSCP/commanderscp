...
/** M1 Graph Core schema (DESIGN.md §4.1-§4.3, §7, §8). See docs/db/schema.md §1. */
...
export const objects = pgTable(
  "objects",
  {
    ...
    // M20.7 (ADR-0031 §6c) — WHY this object is domain-local. See docs/db/schema.md §2.
...
export const roles = pgTable(
  "roles",
  {
     ...
     *  the role. */
    managedByStack: text("managed_by_stack"),
    /** drizzle/0097 — object type ids this role may be bound at. See docs/db/schema.md §3. */
...
export const roleBindings = pgTable(
  ...
  (table) => [
    ...
    /** drizzle/0097 — the NATURAL KEY of a grant. See docs/db/schema.md §4. */
...
export const changes = pgTable(
  "changes",
  {
    ...
    /** THE RECONCILE ROUND-ROBIN CURSOR. See docs/db/schema.md §5. */
...
/** Decision records (DESIGN §10.4) — the explainability funnel. See docs/db/schema.md §6. */
...
// M4 Governance Engine (DESIGN.md §10, BUILD_AND_TEST.md §8 M4). See docs/db/schema.md §7.
...
export const controlRuns = pgTable(
  ...
  (table) => [
    ...
    // 0065 — the composite-FK target for `scan_findings`. See docs/db/schema.md §8.
...
// M5 Campaigns (DESIGN.md §9.5, BUILD_AND_TEST.md §8 M5). See docs/db/schema.md §9.
...
// M6 Federation (DESIGN.md §13, BUILD_AND_TEST.md §8 M6). See docs/db/schema.md §10.
...
export const federationPeers = pgTable(
  "federation_peers",
  {
    ...
    /** M14.1 (ADR-0009, drizzle/0037) — per-peer poke-mode. See docs/db/schema.md §11. */
...
export const syncCursors = pgTable(
  "sync_cursors",
  {
    ...
    /** ONE-SHOT RE-ANCHOR PERMIT. See docs/db/schema.md §12. */
...
/** Bundle-transfer tracking (DESIGN §13). See docs/db/schema.md §13. */
...
// M17.5 — instance-scoped scan-requirement floors (ADR-0016 §3). See docs/db/schema.md §14.
export const scanRequirementFloors = pgTable(
  "scan_requirement_floors",
  {
    tier: text("tier").notNull(),
    // 'local' | 'federated'. See docs/db/schema.md §15.
...
// M21.2 — the DEPENDENCY INVENTORY substrate (ADR-0032 §3/§4/§5/§7). See docs/db/schema.md §16.

/** The identity of ONE MAJOR LINE of one dependency, in one org. See docs/db/schema.md §17. */
export const dependencyLines = pgTable(
  "dependency_lines",
  {
    ...
    // THE PRODUCER LINK USED TO BE HERE. See docs/db/schema.md §18.
...
/** WHICH COMPONENT THIS ORG DECLARES IT PRODUCES ONE COORDINATE. See docs/db/schema.md §19. */
...
export const componentDependencies = pgTable(
  "component_dependencies",
  {
    ...
    /** WHEN THE MANIFEST WAS READ. See docs/db/schema.md §20. */
...
export interface IngestionStampManifest {
  /** THE REPOSITORY THIS ENTRY'S EVIDENCE CAME FROM. See docs/db/schema.md §21. */
  ...
  /** WHEN THE PASS THAT WROTE THIS ENTRY LOOKED, ISO-8601. See docs/db/schema.md §22. */
  readonly at: string;
  /** The ingestion's own sentence about this path. Absent on the ordinary `ok` entry. */
  readonly detail?: string;
}

/** THE DEPENDENCY INGESTION'S RECEIPT, PER COMPONENT. See docs/db/schema.md §23. */
export const dependencyIngestionStamps = pgTable(
  "dependency_ingestion_stamps",
  {
    ...
    /** Per (REPOSITORY, manifest path), sorted. See docs/db/schema.md §24. */
...
/** WHAT COMMANDERSCP ITSELF AUTHORED FOR A DEPENDENCY BUMP. See docs/db/schema.md §25. */
 ...
 */
/** The config-source trigger's durable handoff (migration 0109). See docs/db/schema.md §26. */
...
/** IN-FLIGHT AND CONCLUDED HOOK RUNS. See docs/db/schema.md §27. */
export const pipelineHookRuns = pgTable(
  "pipeline_hook_runs",
  {
    ...
    artifactDigest: text("artifact_digest"),
    commitSha: text("commit_sha"),
    /** NULLABLE, AND THAT IS THE WHOLE DESIGN. See docs/db/schema.md §28. */
