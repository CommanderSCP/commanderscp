...
export class ScpClient {
  ...
  readonly federation = {
    init: async (req: InitFederationRequest): Promise<{ domainId: string; name: string; role: string }> => {
    ...
    getMergedOverlayView: async (baseIdOrUrn: string) => {
      const result = await getMergedOverlayViewRequest({ client: this.client, path: { idOrUrn: baseIdOrUrn } });
      return unwrap(result);
    },
