...
/** THE AWAIT-ORDERING CONFORMANCE SUITE. See docs/runner-launcher.md §330. */
...
export interface LaunchOrderingSubstrate {
  ...
  /** The per-run identity each issued step addressed. See docs/runner-launcher.md §331. */
  ...
  /** Settle the OLDEST still-held occurrence of `kind`. See docs/runner-launcher.md §332. */
  release(kind: RunnerStepKind, failure?: Error): void;
  /** Optional: flush this substrate's deliveries once. See docs/runner-launcher.md §333. */
...
/** ONE CASE'S BOOKKEEPING. See docs/runner-launcher.md §334. */
interface Case {
  hold(kind: RunnerStepKind, count?: number): void;
  release(kind: RunnerStepKind, failure?: Error): void;
...
/** Runs the await-ordering cases against one adapter. See docs/runner-launcher.md §335. */
export function runLaunchOrderingConformanceSuite(
  ...
  describe(`${label}: every step is AWAITED before the next one is issued`, () => {
    ...
    it("THE UNHELD CONTROL — with nothing held, every step is issued in order and run() resolves", async () => {
      // Non-vacuity for every case below. See docs/runner-launcher.md §336.
      const c = open();
  ...
  describe(`${label}: two runs in flight never address each other's container`, () => {
    ...
    it("TWO RUNS AT ONCE — every step of each run addresses ITS OWN identity", async () => {
      // WHAT THIS CATCHES, EXACTLY. See docs/runner-launcher.md §337.
      const c = open();
      c.hold("create", 2);
...
/** Groups issued steps by identity, preserving order. See docs/runner-launcher.md §338. */
