...
// M23.1 PHASE 4. See docs/runner-launcher/docker-adapter.test.md §1.
...
/** PER-STEP FAILURE INJECTION. See docs/runner-launcher/docker-adapter.test.md §2. */
...
/** THE OPTIONS, AS LITERALS. See docs/runner-launcher/docker-adapter.test.md §3. */
...
/** A minimal, entirely explicit spec. See docs/runner-launcher/docker-adapter.test.md §4. */
...
describe("M23.1 conformance: copyOut.when and copyOut.onFailure are independent, and all four arms differ", () => {
  ...
  it("`when: on-success` + `onFailure: swallow` + a FAILED start — NO copy-out; `swallow` never widens WHEN it runs", async () => {
    // THE FOURTH COMBINATION. See docs/runner-launcher/docker-adapter.test.md §5.
    fail("start");
    const result = await createDockerRunnerLauncher("docker").run(
...
// THE LEVERS, FIRED. See docs/runner-launcher/docker-adapter.test.md §6.
...
/** WHAT SURVIVES THE WRAP. See docs/runner-launcher/docker-adapter.test.md §7. */
...
describe("M23.1 conformance: secretEnv never reaches the command line, and never leaves in an error", () => {
  ...
  it("A SUCCEEDED run's stdout and stderr are redacted TOO — success is not a safe channel", async () => {
    // FOUND BY MUTATION, NOT BY READING. See docs/runner-launcher/docker-adapter.test.md §8.
...
// M23.1 PHASE 4. See docs/runner-launcher/docker-adapter.test.md §9.
