...
/** THE PER-TEST BUDGET CENSUS. See docs/source-census.md §18. */

const __dirname = dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = resolve(__dirname, "../../..");

/** THE REVIEWED BUDGETS. See docs/source-census.md §19. */
...
/** Read the declared timeout from source, never by importing. See docs/source-census.md §20. */
function declaredTimeoutMs(source: string): number | null {
  const m = /^\s*testTimeout:\s*([0-9_]+)\s*,?\s*$/m.exec(source);
  return m ? Number(m[1]!.replace(/_/g, "")) : null;
}

/** The same reader for `hookTimeout`. See docs/source-census.md §21. */
...
/** THE SECOND DEADLINE NOBODY DECLARED. See docs/source-census.md §22. */
...
/** THE HOOK BUDGET. See docs/source-census.md §23. */

/** THE REVIEWED UNIT HOOK BUDGET. See docs/source-census.md §24. */
const REVIEWED_HOOK_BUDGETS_MS = new Set([30_000]);

/** The non-unit hook budgets. See docs/source-census.md §25. */
...
const UNIT_HOOK_OVERRIDES: Record<string, { ms: number; why: string }> = {
  ...
  "packages/cli/src/connect-argo-workflows-cli.test.ts": {
    ms: 30_000,
...
/** Every hook call site passing an explicit numeric timeout. See docs/source-census.md §27. */
...
/** The tracked test files a package's UNIT config runs. See docs/source-census.md §28. */
...
describe("no HOOK runs on a deadline nobody chose either", () => {
  ...
  it("EVERY package declares one, including the ones that have no hook today", () => {
    // No allowlist, and that choice is asserted not assumed. See docs/source-census.md §29.
    const files = trackedFiles(REPO_ROOT);
