...
/** The construct tree, in the shape a CDK user expects. See docs/coordination-as-code.md §198. */
...
export abstract class Construct {
  ...
  /** Slash-joined construct-tree path from the root, e.g. See docs/coordination-as-code.md §199. */
  get path(): string {
    const parts: string[] = [];
...
/** A URN plus which typed-registry kind it names. See docs/coordination-as-code.md §200. */
...
/** The placeholder urn a by-name reference resolves to. See docs/coordination-as-code.md §201. */
...
/** Root scope every `Stack` sits under. See docs/coordination-as-code.md §202. */
...
export interface SourceMappingSpec {
  ...
  /**
   * Glob matched against the event's git ref (`refs/heads/main`). See docs/coordination-as-code.md §203.
  ...
  /**
   * Declared reach of the repo. See docs/coordination-as-code.md §204.
...
/** All fields optional. See docs/coordination-as-code.md §206. */
...
/** A named deployable unit (`new Stack('billing-platform')`). See docs/coordination-as-code.md §207. */
...
/** A named deployable unit (`new Stack('billing-platform')`). See docs/coordination-as-code.md §208. */
export class Stack extends Construct {
  ...
  /** `new Stack("platform-estate")` is the ONLY form (D15a). See docs/coordination-as-code.md §209. */
  ...
  /** L1 — the guaranteed raw manifest-entry door. See docs/coordination-as-code.md §210. */
  addManifestEntry(object: ManifestObject): this {
    this.rawObjectDecls.push(object);
    return this;
  }

  /** L1 — the guaranteed raw relationship door. See docs/coordination-as-code.md §211. */
  addRelationship(
    typeId: string,
    from: IResourceRef | string,
    to: IResourceRef | string,
  ...
  /** Declares a `source_mappings` row for `component`. See docs/coordination-as-code.md §212. */
  ...
  /** Declares an `executor_bindings` row for `target`. See docs/coordination-as-code.md §213. */
  /** Declares a `placement` (ADR-0026). See docs/coordination-as-code.md §214. */
  ...
  /** Whether this stack already declares that exact placement. See docs/coordination-as-code.md §215. */
  ...
  /** Declares a binding on a placement, addressed by its pair. See docs/coordination-as-code.md §216. */
  ...
  /** Declares that `component` PRODUCES one dependency coordinate. See docs/coordination-as-code.md §217. */
  ...
  /** Declares that moves beneath this subject need permission. See docs/coordination-as-code.md §218. */
  ...
  /** Pure synth: no clock, no randomness, no ambient input. See docs/coordination-as-code.md §219. */
  /** L1 ESCAPE HATCH for a pipeline hook (D11, D21). See docs/coordination-as-code.md §220. */
  addPipelineHook(
    component: IResourceRef | string,
  ...
  /** L1 ESCAPE HATCH for a rollout declaration. See docs/coordination-as-code.md §221. */
  addRollout(
    component: IResourceRef | string,
  ...
  /** L1 ESCAPE HATCH for a convergence declaration (D25(b)). See docs/coordination-as-code.md §222. */
  /** L1 ESCAPE HATCH for a role binding. See docs/coordination-as-code.md §223. */
  ...
  /** L1 ESCAPE HATCH for an org-defined role. See docs/coordination-as-code.md §224. */
  ...
  synth(): DesiredStateManifest {
    ...
    const candidate = {
      ...
      // OMITTED WHEN EMPTY, like the three above. See docs/coordination-as-code.md §225.
      ...(producers.length > 0 ? { producers } : {}),
      // Omitted when empty, meaning what the sibling's omission does. See docs/coordination-as-code.md §226.
      ...
      // Omitted when empty, and this is the third such collection. See docs/coordination-as-code.md §227.
      ...(pipelineHooks.length > 0 ? { pipelineHooks } : {}),
      ...(rollouts.length > 0 ? { rollouts } : {}),
      ...(convergence.length > 0 ? { convergence } : {})
    };

    // TWO OBJECTS, ONE URN. See docs/coordination-as-code.md §228.
    ...
    // Every synth error names the tree path that produced it. See docs/coordination-as-code.md §229.
...
/** Sorts on the mapping's full identity tuple. See docs/coordination-as-code.md §230. */
...
/** Base class for the 8 typed-registry resource constructs. See docs/coordination-as-code.md §231. */
export class ResourceConstruct<TypeId extends string = string>
  extends Construct
  implements IResourceRef<TypeId>
{
  readonly urn: string;
  /** Public, widened from the earlier round's narrower access. See docs/coordination-as-code.md §232. */
  readonly stack: Stack;

  /** `scope` accepts either a `Stack` directly. See docs/coordination-as-code.md §233. */
  constructor(
  ...
  /** Declares the binding that drives one of these pipelines. See docs/coordination-as-code.md §234. */
  bindsExecutor(spec: ExecutorBindingSpec = {}): this {
    this.stack.addExecutorBinding(this, spec);
    return this;
  }

  // NOTE — there is deliberately NO `coordinates()` fluent method. See docs/coordination-as-code.md §235.

  private manifestOverride: Partial<Omit<ManifestObject, "urn" | "typeId">> = {};

  /** L1 escape hatch, PER-CONSTRUCT (D16(1)). See docs/coordination-as-code.md §236. */
...
/** One tiny factory per resource type, not eight subclasses. See docs/coordination-as-code.md §237. */
...
/** A policy (server-side object type `"policy"`). See docs/coordination-as-code.md §238. */
...
/** A component (server-side object type `"component"`). See docs/coordination-as-code.md §241. */
export class Component extends ResourceConstruct<"component"> {
  ...
  /** Declares a source mapping onto this component (C1). See docs/coordination-as-code.md §242. */
  mapsSource(spec: SourceMappingSpec): this {
    this.stack.addSourceMapping(this, spec);
    return this;
  }

  /** Declares that this component PRODUCES a dependency coordinate. See docs/coordination-as-code.md §243. */
  producesDependency(spec: DependencyProducerSpec): this {
    this.stack.addDependencyProducer(this, spec);
    return this;
  }

  /** Places this component at `deploymentTarget` (ADR-0026). See docs/coordination-as-code.md §244. */
  placeAt(deploymentTarget: IDeploymentTarget | string): Placement {
    return new Placement(this.stack, this, deploymentTarget);
  }
}

/** A placement as a standalone construct (decision Q1's second form). See docs/coordination-as-code.md §245. */
...
export class Placement {
  ...
  /** Declares an `executor_bindings` row on THIS placement. See docs/coordination-as-code.md §246. */
  bindsExecutor(spec: ExecutorBindingSpec = {}): this {
...
// Campaign / Release Topology constructs (M5, BUILD_AND_TEST.md §8). See docs/coordination-as-code.md §247.

/** Resolves a relationship-style reference to a URN string. See docs/coordination-as-code.md §248. */
function resolveUrn(target: IResourceRef | string): string {
  return typeof target === "string" ? target : target.urn;
}

/** Best-effort human-readable LOCATION for a synth validation error. See docs/coordination-as-code.md §249. */
...
/** A named, reusable wave plan. See docs/coordination-as-code.md §250. */
...
export interface CampaignProps extends Omit<ResourceProps, "properties"> {
  /** The objects this campaign fans out to, one change each. See docs/coordination-as-code.md §251. */
  readonly targets: (IResourceRef | string)[];
  /** @default none */
...
/** A coordinated multi-target rollout. See docs/coordination-as-code.md §252. */
