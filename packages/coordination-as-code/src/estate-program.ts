...
/** The shared emitter behind `scp iac export`. See docs/coordination-as-code.md §257. */
...
// D18's loud placeholder. See docs/coordination-as-code.md §258.
...
/** The erased constructor shape every generated class satisfies. See docs/coordination-as-code.md §259. */
...
// The renderer — `--format ts`'s path. See docs/coordination-as-code.md §260.
...
const DEFAULT_HEADER =
  "// GENERATED by `@scp/coordination-as-code` (team-pipeline-iac.md §9/§7) — review before committing.\n" +
  "// Placeholders marked below (if any) make this file FAIL TO TYPECHECK on purpose; fill them in.";

/** §8's commented starter wave topology. See docs/coordination-as-code.md §261. */
export const WAVE_TOPOLOGY_GUIDANCE =
  ...
  '// import { waves } from "@scp/coordination-as-code"; canary percentages within a target are the rollout\n' +
...
export function renderEstateProgram(
      ...
      if (pl.source !== undefined) {
        ...
        placeholderBlocks.push(
          [
            ...
            `// "${pl.kind}" pipeline. @scp/coordination-as-code requires \`repo\` (D18) and never invents one — a`,
  ...
  // Trailing synth + export. See docs/coordination-as-code.md §262.
  ...
  const importLine = `import { ${[...importNames].sort().join(", ")} } from "@scp/coordination-as-code";`;
