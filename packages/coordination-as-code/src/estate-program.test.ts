...
/** Unit coverage for the shared export and scaffold emitter. See docs/coordination-as-code.md §254. */
...
describe("PipelineSpec.topologyUrn — the D5 adoption affordance (never duplicate a live topology)", () => {
  ...
  // MUTATION-WATCHED (restored before commit). See docs/coordination-as-code.md §255.
...
describe("renderEstateProgram", () => {
  it("renders valid-looking construct calls and imports only what it used", () => {
    ...
    expect(placeholderCount).toBe(0);
    expect(source).toContain(
      'import { Cluster, Component, DeploymentTarget, ExecutionSystem, ImagePipeline, Service, Stack } from "@scp/coordination-as-code";'
    );
  ...
  // Mutation-watched: a plausible alternative would be caught. See docs/coordination-as-code.md §256.
