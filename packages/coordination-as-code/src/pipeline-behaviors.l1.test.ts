...
/** THE L1 DOOR FOR THE INCREMENT-8 CONTRACT. See docs/coordination-as-code.md §276. */
describe("@scp/coordination-as-code L1: the increment-8 manifest collections", () => {
  ...
  it("(2) the CONSTRUCT decides the subject — a `componentUrn` smuggled into the spec does not win", () => {
    const { stack, component } = stackWithComponent();
    // The type omits that field, so this is reachable only from JS. See docs/coordination-as-code.md §277.
  ...
  it("(7) a malformed hook is refused by synth WITH the construct path, not accepted onto the wire", () => {
    ...
    stack.addPipelineHook(component, {
      ...
      workflow,
      everySeconds: 300,
      // A RUNTIME contract violation, not a type error. See docs/coordination-as-code.md §278.
      maxAgeSeconds: -1
    });
