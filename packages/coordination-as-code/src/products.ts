...
/** D20's products module. See docs/coordination-as-code.md §300. */
...
/** camelCase JS identifier from any construct id/slug. See docs/coordination-as-code.md §301. */
...
/** Every infra product owned anywhere under `scope`. See docs/coordination-as-code.md §302. */
    ...
    if (existing) {
      throw new Error(
        `Products module for "${scope.path}": both "${existing.urn}" and "${resource.urn}" map to ` +
          ...
          // When two urns print the same, the collision is upstream. See docs/coordination-as-code.md §303.
...
/** Renders `entries` as the module's TypeScript SOURCE TEXT. See docs/coordination-as-code.md §304. */
export function renderProductsModule(entries: readonly ProductEntry[]): string {
  const header =
    "// GENERATED by `@scp/coordination-as-code` synth (team-pipeline-iac.md D20) — DO NOT EDIT BY HAND.\n" +
  ...
  const importLine = `import type { ${usedInterfaces.join(", ")} } from "@scp/coordination-as-code";\n\n`;
