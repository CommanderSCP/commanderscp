...
/** Example-based synth test for a realistic small stack. See docs/coordination-as-code.md §182. */
describe("@scp/coordination-as-code: example stack synth", () => {
...
/** M5 constructs (Campaign, ReleaseTopology). See docs/coordination-as-code.md §183. */
describe("@scp/coordination-as-code: campaign/release-topology synth", () => {
  ...
  it("no construct exposes a membership-edge method — `coordinates` is system-managed (M5 CRITICAL)", () => {
    ...
    // That edge is system-managed, and the apply path refuses it. See docs/coordination-as-code.md §184.
...
/** C1 (docs/proposals/post-import-configuration.md §8). See docs/coordination-as-code.md §185. */
describe("@scp/coordination-as-code constructs: executor bindings on a placement", () => {
...
describe("@scp/coordination-as-code constructs: sourceMappings / executorBindings (C1)", () => {
  ...
  it("declaration ORDER never changes it for mappings differing ONLY by refPattern", () => {
    // THE GAP THE CASE ABOVE LEAVES, and it is not a corner. See docs/coordination-as-code.md §186.
    function build(order: "forward" | "reverse") {
...
describe("@scp/coordination-as-code constructs: placements (C1, ADR-0026)", () => {
  /** A placement is one component at one deployment-target. See docs/coordination-as-code.md §187. */
  function fixture(stackName: string) {
    const stack = new Stack(stackName);
    const service = new Service(stack, "billing", { name: "Billing" });
    const component = new Component(stack, "api", { name: "API", service });
...
/** A dependency subscription is a policy effect, not a row. See docs/coordination-as-code.md §188. */
describe("@scp/coordination-as-code constructs: Policy (M21.6 — a dependency subscription is a policy effect)", () => {
...
describe("@scp/coordination-as-code constructs: dependency producers (ADR-0032 §7e)", () => {
  /** A producer declaration says where this line comes from. See docs/coordination-as-code.md §189. */
  function fixture(stackName: string) {
    const stack = new Stack(stackName);
    const service = new Service(stack, "billing", { name: "Billing" });
    const component = new Component(stack, "api", { name: "API", service });
  ...
  it("omits the collection when empty — and THAT is why deleting your only declaration retracts nothing", () => {
    // Do not "fix" this to emit. See docs/coordination-as-code.md §190.
...
describe("@scp/coordination-as-code constructs: governance:move rungs (ADR-0038 §2)", () => {
  /** A rung says moves beneath this container need permission. See docs/coordination-as-code.md §191. */
  function fixture(stackName: string) {
    const stack = new Stack(stackName);
    const service = new Service(stack, "billing", { name: "Billing" });
  ...
  it("omits the collection when empty — and THAT is why deleting your only rung disables nothing", () => {
    // Do not "fix" this to emit. See docs/coordination-as-code.md §192.
...
/** D16(2) — `fromXxx()` reference statics returning interface types. See docs/coordination-as-code.md §193. */
describe("@scp/coordination-as-code constructs: fromXxx() reference statics (D16(2))", () => {
...
/** D16(1) — the guaranteed L1 escape hatch. See docs/coordination-as-code.md §194. */
describe("@scp/coordination-as-code constructs: the L1 escape hatch (D16(1))", () => {
...
/** D16(5) — construct-path in synth errors. See docs/coordination-as-code.md §195. */
describe("@scp/coordination-as-code: construct-path in synth errors (D16(5))", () => {
...
/** TWO CONSTRUCTS, ONE URN. See docs/coordination-as-code.md §196. */
