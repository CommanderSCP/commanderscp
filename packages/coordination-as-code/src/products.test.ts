...
/** D20's products module. See docs/coordination-as-code.md §296. */
...
describe("@scp/coordination-as-code: collectProducts (D20/D19)", () => {
  ...
  it("throws when two products camelCase to the same identifier, naming both URNs", () => {
    // `camelIdentifier` and `deriveConstructUrn`'s `slugify`. See docs/coordination-as-code.md §297.
...
describe("@scp/coordination-as-code: camelIdentifier", () => {
  it.each([
    ["pay-blue", "payBlue"],
    ["pay_blue", "payBlue"],
    ["Pay Blue", "payBlue"],
...
describe("@scp/coordination-as-code: renderProductsModule / productsModuleSource — content (D20/D24)", () => {
  it("emits an interface-typed const, one readonly field per product, importing only used interfaces", () => {
    ...
    expect(source).toContain(
      'import type { ICluster, IDatabase } from "@scp/coordination-as-code";'
    );
  ...
  it("imports only the interfaces actually used, deduped and sorted", () => {
    ...
    const source = renderProductsModule(entries);
    expect(source).toContain('import type { ICluster } from "@scp/coordination-as-code";');
...
describe("@scp/coordination-as-code: products module determinism (fast-check) — the construct.determinism.test.ts pattern", () => {
  ...
  /** Runs the emitter and returns its output or its refusal. See docs/coordination-as-code.md §298. */
  ...
  it("two independently-built-but-equivalent trees converge to byte-identical output regardless of construction order", () => {
    fc.assert(
      fc.property(productSpecArb, (specs) => {
        ...
        // Both were seeded with the same pairs, constructed differently. See docs/coordination-as-code.md §299.
