...
/**
 * A REAL (non-stubbed) `ScopedHttpClient` for `@scp/plugin-pipeline-generic`'s tests — unlike most
 * plugin unit tests in this repo (webhook-control, fake-executor, federation-https), which stub
 * `ctx.http.request` directly with a hand-written function, THIS package's tests exist
 ...
 * HTTP, since this is a genuinely HTTP-calling plugin.
 *
 ...
 * — empirically confirmed while building this suite (in `@scp/plugin-terraform`, before the M10.6
 * extraction): a bare `nock(url).reply(...)` interceptor plus a `fetch()` call against that same
 * URL throws `TypeError: fetch failed`, never reaching the interceptor. This client is the
 * `node:http`-based sibling of apps/server/src/plugin-host/subprocess-entry.ts's
 * `unscopedFetchHttpClient` — same request/response shape and the same "JSON-parse with raw-text
 * fallback" behavior — swapped only for the transport `nock` can actually see.
 */
